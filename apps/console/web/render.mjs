...
function pageBusinessApplication(application, data) {
  ...
  <script>(()=>{const app=${appJson},canOperate=${canOperate},canApprove=${canApprove},records=document.getElementById('business-records'),pagination=document.getElementById('business-pagination'),message=document.getElementById('business-message'),listState={offset:0,limit:20},esc=v=>String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));const call=async(url,options={})=>{const response=await fetch(url,{headers:{'content-type':'application/json'},...options}),body=await response.json();if(!response.ok)throw new Error(body.reason||body.status||'请求失败');return body;};
