...
function pageDashboard(_model, data) {
  ...
  <section class="portfolio-cockpit"><div class="section-head"><div><span class="eyebrow">AI Business Portfolio</span><h2>集团业务驾驶舱</h2><p>统一查看各平台长期目标、任务进展、阻塞和可验证业务结果。</p></div><a class="quiet-link" href="${routeHref("/domains", data.active_project_id)}">查看平台全景 →</a></div><div class="portfolio-grid">${portfolioCards}</div><p class="portfolio-source" id="portfolio-source">正在读取 Autonomous Kernel…</p></section>
...
function pageDevelopment(data) {
  ...
  <section class="panel"><div class="form-grid"><div><label for="development-project">目标项目</label><select id="development-project">${options}</select></div><div><label for="development-timeout">最长运行时间</label><select id="development-timeout"><option value="600">10 分钟</option><option value="1200">20 分钟</option><option value="1800" selected>30 分钟</option></select></div><div style="grid-column:1/-1"><label for="development-goal">开发目标</label><textarea id="development-goal" rows="4" placeholder="描述要实现的真实功能、业务边界和期望结果"></textarea></div><div style="grid-column:1/-1"><label for="development-paths">允许修改路径（每行一个）</label><textarea id="development-paths" rows="3" placeholder="apps/console/web/render.mjs"></textarea></div><div style="grid-column:1/-1"><label for="development-acceptance">验收标准（每行一项）</label><textarea id="development-acceptance" rows="3" placeholder="页面可以完成目标操作&#10;定向测试通过&#10;不修改允许路径之外的文件"></textarea></div><div style="grid-column:1/-1"><label for="development-commands">验证命令（每行一个）</label><textarea id="development-commands" rows="3">git diff --check</textarea></div></div><div class="button-row" style="justify-content:space-between;margin-top:14px"><p id="development-message" class="help">创建后先进入澄清或审批状态；批准前不会启动 Codex。</p><button id="development-create" class="primary-action" type="button">创建真实开发任务</button></div></section>
  ...
  <script>(()=>{const box=document.getElementById('development-jobs'),message=document.getElementById('development-message'),workerBadge=document.getElementById('development-worker'),dialog=document.getElementById('development-artifact-dialog');const esc=v=>String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));const call=async(url,options={})=>{const response=await fetch(url,{headers:{'content-type':'application/json'},...options}),body=await response.json();if(!response.ok)throw new Error(body.reason||body.status||'请求失败');return body;};const statusLabel={NEEDS_CLARIFICATION:'需要澄清',PENDING_APPROVAL:'等待批准',QUEUED:'等待 Worker',RUNNING:'真实执行中',AWAITING_DIFF_APPROVAL:'等待 Diff 审批',NEEDS_REWORK:'需要返工',FAILED:'失败',CANCELLED:'已取消',COMMITTED:'已提交'};
  const render=body=>{const worker=body.worker;workerBadge.textContent='● '+(worker.status==='IDLE'?'Worker 在线':worker.status==='BUSY'?'Worker 执行中':'Worker 离线');workerBadge.className='status-label '+(worker.status==='IDLE'?'pass':worker.status==='BUSY'?'local':'blocked');if(!body.jobs.length){box.innerHTML='<div class="panel empty-state"><strong>还没有真实开发任务</strong></div>';return;}box.innerHTML=body.jobs.map(job=>'<article class="panel" style="margin-bottom:14px"><div class="section-head"><div><span class="eyebrow">'+esc(job.projectId)+' · '+esc(job.id)+'</span><h2>'+esc(job.goal)+'</h2><p class="help">'+esc(job.allowedPaths.join(' · '))+'</p></div><span class="status-label '+(job.status==='COMMITTED'||job.status==='AWAITING_DIFF_APPROVAL'?'pass':job.status==='RUNNING'||job.status==='QUEUED'?'local':job.status==='FAILED'||job.status==='NEEDS_REWORK'?'blocked':'pending')+'">● '+esc(statusLabel[job.status]||job.status)+'</span></div><div class="grid"><div class="metric"><span>真实 Agent</span><strong>'+job.agentInstances.length+' / 4</strong></div><div class="metric"><span>Input Token</span><strong>'+Number(job.tokenUsage.inputTokens||0).toLocaleString()+'</strong></div><div class="metric"><span>Output Token</span><strong>'+Number(job.tokenUsage.outputTokens||0).toLocaleString()+'</strong></div><div class="metric"><span>变更文件</span><strong>'+job.changedPaths.length+'</strong></div></div>'+(job.clarification.questions.length&&!job.clarification.answer?'<div class="attention-card panel" style="margin-top:12px"><strong>'+esc(job.clarification.questions.join(' '))+'</strong><textarea data-clarification="'+esc(job.id)+'" rows="2" placeholder="输入补充说明"></textarea></div>':'')+'<div class="simple-list" style="margin-top:12px">'+job.agentInstances.map(agent=>'<div class="simple-row"><div><strong>'+esc(agent.role)+'</strong><small>'+esc((agent.skillPack||[]).join(' · '))+'</small></div><div><span>'+esc(agent.status)+'</span><small>'+Number(agent.tokenUsage?.input_tokens||0).toLocaleString()+' tokens</small></div></div>').join('')+'</div><div class="button-row" style="margin-top:12px;flex-wrap:wrap">'+job.artifacts.map(a=>'<button class="secondary" data-artifact="'+esc(a.id)+'" data-id="'+esc(job.id)+'">'+esc(a.type)+'</button>').join('')+'<span style="flex:1"></span>'+(job.status==='NEEDS_CLARIFICATION'?'<button class="primary" data-action="clarify" data-id="'+esc(job.id)+'">提交澄清</button>':'')+(job.status==='PENDING_APPROVAL'||job.status==='PAUSED'?'<button class="primary" data-action="approve" data-id="'+esc(job.id)+'">批准真实执行</button>':'')+(job.status==='QUEUED'||job.status==='RUNNING'?'<button class="danger" data-action="cancel" data-id="'+esc(job.id)+'">取消</button>':'')+(job.status==='AWAITING_DIFF_APPROVAL'?'<button class="primary" data-action="commit" data-id="'+esc(job.id)+'">批准 Diff 并 Commit</button>':'')+'</div></article>').join('');};const refresh=async()=>render(await call('/api/development/jobs'));
  document.getElementById('development-create').onclick=async()=>{const button=document.getElementById('development-create'),lines=id=>document.getElementById(id).value.split('\n').map(v=>v.trim()).filter(Boolean);button.disabled=true;try{await call('/api/development/jobs',{method:'POST',body:JSON.stringify({projectId:document.getElementById('development-project').value,goal:document.getElementById('development-goal').value,allowedPaths:lines('development-paths'),acceptanceCriteria:lines('development-acceptance'),acceptanceCommands:lines('development-commands'),maxRuntimeSeconds:Number(document.getElementById('development-timeout').value)})});message.textContent='任务已创建，请完成澄清或批准。';await refresh();}catch(error){message.textContent=error.message;}finally{button.disabled=false;}};box.onclick=async event=>{const artifact=event.target.closest('[data-artifact]'),action=event.target.closest('[data-action]');if(artifact){const result=await call('/api/development/jobs/'+encodeURIComponent(artifact.dataset.id)+'/artifacts/'+encodeURIComponent(artifact.dataset.artifact));document.getElementById('development-artifact-title').textContent=result.artifact.type;document.getElementById('development-artifact-content').textContent=result.content;dialog.showModal();return;}if(!action)return;if((action.dataset.action==='approve'||action.dataset.action==='commit')&&!confirm(action.dataset.action==='approve'?'确认启动真实 Codex 四 Agent 流程？':'确认当前 Diff 后创建本地 Commit？'))return;const body=action.dataset.action==='clarify'?{answer:document.querySelector('[data-clarification="'+CSS.escape(action.dataset.id)+'"]').value}:{};try{await call('/api/development/jobs/'+encodeURIComponent(action.dataset.id)+'/'+action.dataset.action,{method:'POST',body:JSON.stringify(body)});await refresh();}catch(error){message.textContent=error.message;}};document.getElementById('development-artifact-close').onclick=()=>dialog.close();document.getElementById('development-refresh').onclick=()=>refresh().catch(error=>message.textContent=error.message);refresh().catch(error=>message.textContent=error.message);setInterval(()=>refresh().catch(error=>message.textContent=error.message),4000);})();</script>`;
}

function pageMyWork() {
  return `<section class="product-hero"><div><span class="eyebrow">My Work</span><h2>我的工作</h2><p>集中处理分配给我的业务待办、我委派给 Agent 的工作、待审批事项、阻塞和分析结果。这里展示业务工作，不暴露内部租约、进程或 Fencing 信息。</p></div><button id="work-refresh" class="secondary" type="button">刷新</button></section>
  <section class="summary-grid" id="work-summary"></section><section id="work-approval-section"><div class="section-head"><div><h2>待我审批</h2><p class="help">审批绑定业务对象版本，单据变化后旧审批自动失效。</p></div></div><div id="work-approvals" class="panel"><div class="empty-state"><strong>正在读取审批</strong></div></div></section><section><div class="section-head"><div><h2>工作队列</h2><p class="help">每项工作都可返回对应业务单据。</p></div></div><div id="work-list" class="panel"><div class="empty-state"><strong>正在读取工作</strong></div></div></section>
  <script>(()=>{const list=document.getElementById('work-list'),approvals=document.getElementById('work-approvals'),approvalSection=document.getElementById('work-approval-section'),summary=document.getElementById('work-summary'),esc=v=>String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));async function load(){const response=await fetch('/api/work',{cache:'no-store'}),body=await response.json();if(!response.ok)throw new Error(body.reason||'工作队列不可用');const cards=[['全部工作',body.summary.total],['我的待办',body.summary.human],['Agent 工作',body.summary.agent],['待我审批',body.summary.pendingApprovals]];summary.innerHTML=cards.map(x=>'<div class="summary-card"><span>'+x[0]+'</span><strong>'+x[1]+'</strong></div>').join('');approvalSection.hidden=!body.canApprove;approvals.innerHTML=body.approvals.length?'<table><thead><tr><th>业务单据</th><th>申请人</th><th>审批动作</th><th>版本</th><th></th></tr></thead><tbody>'+body.approvals.map(item=>'<tr><td><strong>'+esc(item.businessObject.title)+'</strong><small>'+esc(item.businessObject.displayKey)+'</small></td><td>'+esc(item.requestedBy)+'</td><td>'+esc(item.fromStatus)+' → '+esc(item.requestedStatus)+'</td><td>v'+esc(item.objectVersion)+'</td><td><a class="quiet-link" href="'+esc(item.businessObject.href)+'">打开并审批 →</a></td></tr>').join('')+'</tbody></table>':'<div class="empty-state"><strong>当前没有待审批事项</strong></div>';list.innerHTML=body.items.length?'<table><thead><tr><th>工作</th><th>业务单据</th><th>执行者</th><th>状态</th><th></th></tr></thead><tbody>'+body.items.map(item=>'<tr><td><strong>'+esc(item.title)+'</strong></td><td>'+esc(item.businessObject.displayKey)+'</td><td>'+(item.assignmentType==='AGENT'?'智能助手':'业务人员')+'</td><td>'+esc(item.status)+'</td><td><a class="quiet-link" href="'+esc(item.businessObject.href)+'">打开单据 →</a></td></tr>').join('')+'</tbody></table>':'<div class="empty-state"><strong>当前没有待办</strong>业务系统分配的人工和 Agent 工作会出现在这里。</div>';}document.getElementById('work-refresh').onclick=()=>load().catch(error=>list.textContent=error.message);load().catch(error=>list.textContent=error.message);})();</script>`;
}

function pageBusinessApplication(application, data) {
  const applicationView={...application,objectTypes:application.objectTypes.map(type=>({...type,schema:getBusinessObjectDefinition(application.id,type.id)}))};
  ...
  const typeOptions=application.objectTypes.map(type=>`<option value="${escapeHtml(type.id)}">${escapeHtml(type.name)}</option>`).join("");
  ...
  <section><div class="section-head"><div><h2>业务台账</h2><p class="help">传统业务记录与状态。智能工作始终关联到这里的正式单据。</p></div><button id="business-refresh" class="secondary" type="button">刷新</button></div><div id="business-records" class="panel"><div class="empty-state"><strong>正在读取业务记录</strong></div></div></section>
  ...
  <script>(()=>{const app=${appJson},canOperate=${canOperate},canApprove=${canApprove},records=document.getElementById('business-records'),message=document.getElementById('business-message'),esc=v=>String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));const call=async(url,options={})=>{const response=await fetch(url,{headers:{'content-type':'application/json'},...options}),body=await response.json();if(!response.ok)throw new Error(body.reason||body.status||'请求失败');return body;};
  ...
  async function openDetail(id){const body=await call('/api/business/applications/'+encodeURIComponent(app.id)+'/records/'+encodeURIComponent(id)),row=body.record,dialog=document.getElementById('business-detail-dialog'),content=document.getElementById('business-detail-content');document.getElementById('business-detail-key').textContent=row.displayKey+' · v'+row.version;document.getElementById('business-detail-title').textContent=row.title;const fields=row.schema.fields.map(field=>'<div class="simple-row"><strong>'+esc(field.label)+'</strong><span>'+esc(row.fields?.[field.key]??'—')+'</span></div>').join(''),work=body.workItems.length?body.workItems.map(item=>'<div class="simple-row"><div><strong>'+esc(item.title)+'</strong><small>'+esc(item.assignmentType==='AGENT'?'智能助手 / '+item.assigneeId:'业务人员 / '+item.assigneeId)+'</small></div><div><span class="status-label '+(item.status==='BLOCKED'?'blocked':item.status==='WAITING_APPROVAL'?'pending':'local')+'">'+esc(item.status)+'</span><small>'+(item.kernelGoalId?'Goal '+esc(item.kernelGoalId):'尚未进入 Kernel')+'</small></div></div>').join(''):'<div class="empty-state"><strong>尚无工作项</strong></div>',approvals=body.approvals.length?body.approvals.map(item=>'<div class="simple-row"><div><strong>'+esc(item.fromStatus)+' → '+esc(item.requestedStatus)+'</strong><small>'+esc(item.requestedBy)+' · '+esc(new Date(item.createdAt).toLocaleString())+'</small></div><div><span class="status-label '+(item.status==='APPROVED'?'pass':item.status==='REJECTED'?'blocked':'pending')+'">'+esc(item.status)+'</span>'+(canApprove&&item.status==='PENDING'?'<div class="button-row"><button class="primary" data-approval-decision="APPROVED" data-approval-id="'+esc(item.id)+'" data-record-id="'+esc(row.id)+'">批准</button><button class="secondary" data-approval-decision="REJECTED" data-approval-id="'+esc(item.id)+'" data-record-id="'+esc(row.id)+'">驳回</button></div>':'')+'</div></div>').join(''):'<div class="empty-state"><strong>尚无审批</strong></div>',events=body.timeline.length?body.timeline.map(event=>'<div class="simple-row"><div><strong>'+esc(event.type)+'</strong><small>'+esc(event.actorId)+' · '+esc(new Date(event.createdAt).toLocaleString())+'</small></div><div><span>v'+esc(event.objectVersion??'—')+'</span><small>'+esc(Object.values(event.payload||{}).filter(value=>typeof value!=='object').join(' · '))+'</small></div></div>').join(''):'<div class="empty-state"><strong>尚无事件</strong></div>';content.innerHTML='<section class="summary-grid"><div class="summary-card"><span>当前状态</span><strong>'+esc(row.status)+'</strong><small>正式业务状态</small></div><div class="summary-card"><span>负责人</span><strong>'+esc(row.ownerId)+'</strong><small>业务责任人</small></div><div class="summary-card"><span>工作项</span><strong>'+body.workItems.length+'</strong><small>人工与 Agent</small></div><div class="summary-card"><span>待审批</span><strong>'+body.approvals.filter(item=>item.status==='PENDING').length+'</strong><small>必须人工决策</small></div></section><section class="panel"><h3>业务字段</h3><div class="simple-list">'+fields+'</div></section><section class="panel"><h3>业务审批</h3><div class="simple-list">'+approvals+'</div></section><section class="panel"><h3>工作与 Runner 证据</h3><div class="simple-list">'+work+'</div></section><section class="panel"><h3>业务时间线</h3><div class="simple-list">'+events+'</div></section>';dialog.showModal();}
  async function load(){const body=await call('/api/business/applications/'+encodeURIComponent(app.id)+'/records'),backend=document.getElementById('business-backend');backend.textContent=body.backend==='POSTGRESQL'?'● 事务数据库':'● 本地数据';backend.className='status-label '+(body.backend==='POSTGRESQL'?'pass':'pending');records.innerHTML=body.records.length?'<table><thead><tr><th>编号</th><th>业务事项</th><th>关键数据</th><th>负责人</th><th>状态</th><th>下一步</th></tr></thead><tbody>'+body.records.map(row=>{const detail=Object.entries(row.fields||{}).slice(0,3).map(([key,value])=>{const label=row.schema.fields.find(field=>field.key===key)?.label||key;return esc(label)+'：'+esc(value);}).join(' · '),review=row.schema.agentReviewStatus,canDelegate=row.availableTransitions.includes(review),actions=row.availableTransitions.map(status=>'<button class="secondary" '+(row.status==='WAITING_APPROVAL'?'data-request-approval="'+esc(status)+'"':'data-transition="'+esc(status)+'"')+' data-id="'+esc(row.id)+'" data-version="'+row.version+'">'+(row.status==='WAITING_APPROVAL'?'提请 '+esc(status):esc(status))+'</button>').join('');return '<tr id="record-'+esc(row.id)+'"><td><button class="quiet-link" data-detail="'+esc(row.id)+'">'+esc(row.displayKey)+'</button></td><td><strong>'+esc(row.title)+'</strong><small>'+esc(app.objectTypes.find(x=>x.id===row.objectType)?.name||row.objectType)+'</small></td><td>'+detail+'</td><td>'+esc(row.ownerId)+'</td><td><span class="status-label local">'+esc(row.status)+'</span></td><td>'+(canOperate?'<div class="button-row">'+actions+'<button class="secondary" data-assign="HUMAN" data-id="'+esc(row.id)+'">分配给我</button>'+(canDelegate?'<button class="primary" data-assign="AGENT" data-id="'+esc(row.id)+'">委派智能助手</button>':'')+'</div>':'—')+'</td></tr>';}).join('')+'</tbody></table>':'<div class="empty-state"><strong>还没有业务记录</strong>从新建业务记录开始；即使智能 Runtime 关闭，业务台账仍然可用。</div>';}
  if(canOperate){const type=document.getElementById('business-type');type.onchange=renderFields;renderFields();document.getElementById('business-create').onclick=async()=>{try{const fields={};document.querySelectorAll('[data-business-field]').forEach(input=>{if(input.value!=='')fields[input.dataset.businessField]=input.value;});await call('/api/business/applications/'+encodeURIComponent(app.id)+'/records',{method:'POST',body:JSON.stringify({objectType:type.value,title:document.getElementById('business-title').value,displayKey:document.getElementById('business-ref').value,fields})});message.textContent='业务记录已保存。';document.getElementById('business-title').value='';renderFields();await load();}catch(error){message.textContent=error.message;}};}records.onclick=async event=>{const detail=event.target.closest('[data-detail]'),approval=event.target.closest('[data-request-approval]'),assign=event.target.closest('[data-assign]'),transition=event.target.closest('[data-transition]');try{if(detail){await openDetail(detail.dataset.detail);return;}if(approval)await call('/api/business/applications/'+encodeURIComponent(app.id)+'/records/'+encodeURIComponent(approval.dataset.id)+'/approvals',{method:'POST',body:JSON.stringify({expectedVersion:Number(approval.dataset.version),requestedStatus:approval.dataset.requestApproval})});if(assign)await call('/api/business/applications/'+encodeURIComponent(app.id)+'/work',{method:'POST',body:JSON.stringify({businessObjectId:assign.dataset.id,assignmentType:assign.dataset.assign,assigneeId:assign.dataset.assign==='AGENT'?'agent-business-operator':'${escapeHtml(data.renderAuth.user?.user_id??"unknown")}'})});if(transition)await call('/api/business/applications/'+encodeURIComponent(app.id)+'/records/'+encodeURIComponent(transition.dataset.id)+'/transition',{method:'POST',body:JSON.stringify({expectedVersion:Number(transition.dataset.version),status:transition.dataset.transition})});await load();}catch(error){if(message)message.textContent=error.message;}};document.getElementById('business-detail-content').onclick=async event=>{const button=event.target.closest('[data-approval-decision]');if(!button)return;try{await call('/api/business/applications/'+encodeURIComponent(app.id)+'/approvals/'+encodeURIComponent(button.dataset.approvalId)+'/decision',{method:'POST',body:JSON.stringify({decision:button.dataset.approvalDecision})});await load();await openDetail(button.dataset.recordId);}catch(error){if(message)message.textContent=error.message;}};document.getElementById('business-detail-close').onclick=()=>document.getElementById('business-detail-dialog').close();document.getElementById('business-refresh').onclick=()=>load().catch(error=>records.textContent=error.message);load().then(()=>{const requested=new URLSearchParams(location.search).get('record');if(requested)return openDetail(requested);}).catch(error=>records.textContent=error.message);})();</script>`;
}
