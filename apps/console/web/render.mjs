...
function navIcon(id) {
  const paths = {
    ...
    capabilityApps: '<path d="M3 4h6v5H3zM11 4h6v5h-6zM3 11h6v5H3zM11 11h6v5h-6z"/><path d="M9 6.5h2M6 9v2m8-2v2M9 13.5h2"/>',
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    html[data-theme] .product-hero { border-color:var(--theme-border); background:linear-gradient(135deg,var(--theme-surface) 0%,color-mix(in srgb,var(--theme-accent-soft) 72%,var(--theme-surface)) 100%); color:var(--theme-text); box-shadow:0 18px 52px var(--theme-shadow); }
    html[data-theme] .product-hero::after { background:radial-gradient(circle,color-mix(in srgb,var(--theme-accent) 22%,transparent),transparent 68%); filter:none; }
    html[data-theme] .product-hero h2 { color:var(--theme-text); }
    html[data-theme] .product-hero p { color:var(--theme-text-soft); }
    html[data-theme] .domain-card h3,html[data-theme] .application-suite h2,html[data-theme] .domain-card small { color:var(--theme-text); }
    html[data-theme] .domain-card p,html[data-theme] .application-suite-head p { color:var(--theme-text-soft); }
    html[data-theme] .application-badge,html[data-theme] .domain-mark,html[data-theme] .domain-icon { border-color:var(--theme-border); background:var(--theme-accent-soft); color:var(--theme-accent); box-shadow:none; }
    html[data-theme] .advanced-section > summary,html[data-theme] .details-drawer summary { color:var(--theme-text); background:var(--theme-surface-soft); }
    html[data-theme] .advanced-section > summary:hover,html[data-theme] .details-drawer summary:hover { color:var(--theme-accent-strong); background:var(--theme-accent-soft); }
    html[data-theme] .simple-row,html[data-theme] .activity-item { background:var(--theme-surface); color:var(--theme-text); }
    html[data-theme] .simple-row:hover,html[data-theme] .activity-item:hover,html[data-theme] tbody tr:hover { background:var(--theme-accent-soft); }
...
function pageBusinessApplication(application, data) {
  ...
  const appJson=JSON.stringify(applicationView).replaceAll("<","\\u003c"),capabilities=data.renderAuth.capabilities??[],canOperate=capabilities.some(value=>value==="*"||value==="business.operate"),canApprove=capabilities.some(value=>value==="*"||value==="proposal.approve"),canRetryDelivery=capabilities.some(value=>value==="*"||value==="business.work.control")&&capabilities.some(value=>value==="*"||value==="sales.read"),canReconcileDelivery=capabilities.some(value=>value==="*"||value==="business.manage")&&canApprove;
  ...
  const growthDeliveryControl=application.id==="ai-growth-sales-platform"?`<section><div class="section-head"><div><span class="eyebrow">Governed Delivery Operations</span><h2>外部投递与对账</h2><p class="help">只读展示发布与商业交接的执行状态、重试资格和对账结果。</p></div><span id="growth-delivery-source" class="pill">正在读取</span></div><div id="growth-delivery-summary" class="summary-grid"></div><div id="growth-delivery-items" class="panel" style="margin-top:12px"></div></section><script>(()=>{const esc=v=>String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));fetch('/api/business/growth-sales/delivery-dashboard',{cache:'no-store'}).then(r=>r.ok?r.json():Promise.reject(new Error('delivery unavailable'))).then(body=>{document.getElementById('growth-delivery-summary').innerHTML=[['全部投递',body.summary.total],['执行中',body.summary.running],['可重试',body.summary.retryable],['需要处理',body.summary.actionRequired]].map(item=>'<div class="summary-card"><span>'+item[0]+'</span><strong>'+esc(item[1])+'</strong></div>').join('');document.getElementById('growth-delivery-items').innerHTML='<div class="section-head"><h3>异常与待对账</h3><span class="status-label '+(body.items.length?'blocked':'pass')+'">'+esc(body.items.length)+' 项</span></div><div class="simple-list">'+(body.items.length?body.items.map(item=>'<div class="simple-row"><div><strong>'+esc(item.operationType)+' · '+esc(item.capability)+'</strong><small>'+esc(item.adapterId)+' · 尝试 '+esc(item.attempts)+' / '+esc(item.maxAttempts)+'</small></div><div><span class="status-label '+(item.status==='FAILED'||item.reconciliationStatus==='MISMATCH'?'blocked':'pending')+'">'+esc(item.status)+'</span><small>对账 '+esc(item.reconciliationStatus)+(item.errorCode?' · '+esc(item.errorCode):'')+'</small></div></div>').join(''):'<div class="empty-state"><strong>当前没有投递异常</strong></div>')+'</div><p class="help">本视图不暴露凭证、审批引用、请求指纹、来源引用或错误正文，也不直接执行重试。</p>';document.getElementById('growth-delivery-source').textContent='事务数据库 · '+new Date(body.generatedAt).toLocaleString();}).catch(()=>{document.getElementById('growth-delivery-items').innerHTML='<div class="empty-state"><strong>投递账本尚未接入或迁移</strong></div>';document.getElementById('growth-delivery-source').textContent='数据源不可用';});})();</script>`:"";
  const growthPilotReadiness=application.id==="ai-growth-sales-platform"?`<section><div class="section-head"><div><span class="eyebrow">Pilot Activation Evidence</span><h2>KingTurf Pilot 就绪度</h2><p class="help">把版本化实施证据与真实生产激活条件分开呈现；代码就绪不等于获准上线。</p></div><span id="growth-pilot-status" class="status-label pending">正在评估</span></div><div id="growth-pilot-summary" class="summary-grid"></div><div id="growth-pilot-blockers" class="panel" style="margin-top:12px"></div></section><script>(()=>{const esc=v=>String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c])),labels={WEBSITE_CREDENTIAL_REFERENCE:'网站凭据引用未配置',PUBLISHING_CREDENTIAL_REFERENCE:'发布凭据引用未配置',BUSINESS_CREDENTIAL_REFERENCE:'业务系统凭据引用未配置',CONNECTOR_HEALTH:'连接器健康尚未通过',DATA_OWNER_APPROVAL:'数据负责人尚未批准',NO_IDENTITY_REVIEW_BACKLOG:'仍有身份复核积压或案件表未观测',NO_DELIVERY_FAILURES:'仍有失败投递或账本未观测',RECONCILIATION_MATCHED:'仍有对账不一致或账本未观测',PRODUCTION_FEATURE_FLAG:'生产特性开关未开启',RUNTIME_ACTIVATION_GATE:'Runtime 激活 Gate 未通过',EXPLICIT_PRODUCTION_AUTHORIZATION:'缺少本次明确生产授权'};fetch('/api/business/growth-sales/pilot-readiness',{cache:'no-store'}).then(r=>r.ok?r.json():Promise.reject(new Error('pilot readiness unavailable'))).then(body=>{const passed=body.implementation.filter(x=>x.pass).length+body.activation.filter(x=>x.pass).length,total=body.implementation.length+body.activation.length,status=document.getElementById('growth-pilot-status');status.textContent=body.status;status.className='status-label '+(body.activationReady?'pass':'blocked');document.getElementById('growth-pilot-summary').innerHTML=[['实施证据',body.implementationReady?'READY':'BLOCKED'],['生产激活',body.activationReady?'READY':'BLOCKED'],['检查通过',passed+' / '+total],['安全边界',body.safety.externalWritesPerformed?'异常':'零外部写入']].map(item=>'<div class="summary-card"><span>'+item[0]+'</span><strong>'+esc(item[1])+'</strong></div>').join('');document.getElementById('growth-pilot-blockers').innerHTML='<div class="section-head"><h3>生产激活阻断</h3><span class="status-label '+(body.blockers.length?'blocked':'pass')+'">'+esc(body.blockers.length)+' 项</span></div><div class="simple-list">'+(body.blockers.length?body.blockers.map(id=>'<div class="simple-row"><strong>'+esc(labels[id]||id)+'</strong><span>'+esc(id)+'</span></div>').join(''):'<div class="empty-state"><strong>所有激活证据已通过，仍需按治理流程执行</strong></div>')+'</div><p class="help">只读证据投影：运营计数来自租户范围内的身份复核案件与投递账本；数据源缺失保持未知并阻断。这里不读取凭据值、不调用连接器、不修改生产，也不提供 Runtime 或 Pilot 激活操作。</p>';}).catch(()=>{document.getElementById('growth-pilot-status').textContent='证据不可用';document.getElementById('growth-pilot-status').className='status-label blocked';document.getElementById('growth-pilot-blockers').innerHTML='<div class="empty-state"><strong>Pilot 就绪证据暂不可用</strong></div>';});})();</script>`:"";
  const growthIdentityReviews=application.id==="ai-growth-sales-platform"?`<section><div class="section-head"><div><span class="eyebrow">Identity Governance</span><h2>身份冲突人工复核</h2><p class="help">只读展示因多个确定性身份指向不同 Lead 而被回滚的导入案件。</p></div><span id="growth-identity-review-source" class="pill">正在读取</span></div><div id="growth-identity-review-items" class="panel"></div></section><script>(()=>{const esc=v=>String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));fetch('/api/business/growth-sales/identity-reviews?status=OPEN',{cache:'no-store'}).then(r=>r.ok?r.json():Promise.reject(new Error('identity review unavailable'))).then(body=>{document.getElementById('growth-identity-review-source').textContent='事务数据库 · '+body.items.length+' 项待复核';document.getElementById('growth-identity-review-items').innerHTML='<div class="simple-list">'+(body.items.length?body.items.map(item=>'<div class="simple-row"><div><strong>'+esc(item.id)+'</strong><small>身份类型 '+esc(item.identityTypes.join(' / '))+' · 候选 Lead '+esc(item.candidateLeadIds.length)+' 个</small></div><div><span class="status-label blocked">'+esc(item.status)+'</span><small>版本 '+esc(item.version)+'</small></div></div>').join(''):'<div class="empty-state"><strong>当前没有待复核身份冲突</strong></div>')+'</div><p class="help">页面不显示原始邮箱、电话、域名或外部表单编号，也不提供合并按钮；人工决策通过独立 RBAC、精确版本和不可变审计 API 完成。</p>';}).catch(()=>{document.getElementById('growth-identity-review-source').textContent='数据源不可用';document.getElementById('growth-identity-review-items').innerHTML='<div class="empty-state"><strong>身份复核案件暂不可用</strong></div>';});})();</script>`:"";
  const growthConnectorActivationPreflights=application.id==="ai-growth-sales-platform"?`<section><div class="section-head"><div><span class="eyebrow">Production Operations Handoff</span><h2>连接器激活只读预检</h2><p class="help">把已进入审批流程的激活申请交给生产运维核验；本视图不执行激活。</p></div><span id="growth-activation-preflight-source" class="pill">正在读取</span></div><div id="growth-activation-preflight-summary" class="summary-grid"></div><div id="growth-activation-preflight-items" class="panel" style="margin-top:12px"></div></section><script>(()=>{const esc=v=>String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c])),labels={ACTIVATION_REQUEST_NOT_APPROVED:'申请尚未批准',ACTIVATION_APPROVAL_NOT_PROVEN:'审批证据未确认',ACTIVATION_TENANT_MISMATCH:'租户不匹配',EXPLICIT_PRODUCTION_AUTHORIZATION_INVALID:'明确生产授权无效',ACTIVATION_AUTHORIZATION_EXPIRED:'明确生产授权已过期',CONNECTOR_BINDING_NOT_FOUND:'连接器绑定不存在',CONNECTOR_BINDING_ALREADY_ENABLED:'连接器已启用',CONNECTOR_KIND_MISMATCH:'连接器类型不匹配',CONNECTOR_BINDING_VERSION_CONFLICT:'绑定版本已变化',CONNECTOR_HEALTH_NOT_FRESH:'健康证据缺失或过期'};fetch('/api/business/growth-sales/connector-activation-preflights',{cache:'no-store'}).then(r=>r.ok?r.json():Promise.reject(new Error('activation preflight unavailable'))).then(body=>{document.getElementById('growth-activation-preflight-source').textContent='事务数据库 · '+new Date(body.generatedAt).toLocaleString();document.getElementById('growth-activation-preflight-summary').innerHTML=[['待核验',body.summary.total],['可交接',body.summary.ready],['仍阻断',body.summary.blocked],['全局生产操作',body.productionOperations.status]].map(item=>'<div class="summary-card"><span>'+item[0]+'</span><strong>'+esc(item[1])+'</strong></div>').join('');document.getElementById('growth-activation-preflight-items').innerHTML='<div class="attention-card panel"><strong>Production Ops '+esc(body.productionOperations.gateStatus)+'</strong><p>'+esc(body.productionOperations.gateId||'生产操作闸门缺失')+' · '+esc(body.productionOperations.productionOperations)+' · '+esc(body.productionOperations.reasons.join(' / '))+'</p></div><div class="simple-list">'+(body.items.length?body.items.map(item=>'<div class="simple-row"><div><strong>'+esc(item.binding.kind||'未绑定')+' · '+esc(item.activationRequest.id)+'</strong><small>申请 v'+esc(item.activationRequest.version)+' · 绑定 v'+esc(item.binding.version??'—')+' · 健康 '+esc(item.binding.healthStatus??'未观测')+'</small></div><div><span class="status-label '+(item.status==='READY'?'pass':'blocked')+'">'+esc(item.status)+'</span><small>'+(item.reasons.length?esc(item.reasons.map(code=>labels[code]||code).join(' · ')):'等待独立受权运维执行')+'</small></div></div>').join(''):'<div class="empty-state"><strong>当前没有待核验激活申请</strong></div>')+'</div><p class="help">全局 Production Ops 策略当前保持 disabled。这里不显示 Credential Reference、明确授权编号、审批评论或原始健康证据；页面没有激活按钮。申请 READY 也不覆盖全局闸门。</p>';}).catch(()=>{document.getElementById('growth-activation-preflight-source').textContent='数据源不可用';document.getElementById('growth-activation-preflight-items').innerHTML='<div class="empty-state"><strong>激活预检数据暂不可用</strong></div>';});})();</script>`:"";
  ...
  ${smartParkControl}
  ${growthControl}
  ${growthPilotReadiness}
  ${growthConnectorActivationPreflights}
  ${growthIdentityReviews}
  ${growthDeliveryControl}
...
function pageCapabilityApps(data) {
  const apps = data.managedCapabilityApps?.apps ?? [];
  const ready = apps.filter((app) => app.status === "READY").length;
  const projectCount = apps.reduce((sum, app) => sum + (app.projects?.length ?? 0), 0);
  const panels = apps.map((app) => {
    const projects = (app.projects ?? []).map((project) => {
      const state = app.project_states?.[project.project_id] ?? null, receipt = state?.studio_handoff ?? null, review = state?.artifacts?.final_review ?? null, report = state?.artifacts?.render_report?.outputs?.[0] ?? null;
      const rawRenderPath = String(state?.media?.renders?.[0]?.path ?? ""), renderPath = rawRenderPath.includes("/renders/") ? `renders/${rawRenderPath.split("/renders/").pop()}` : rawRenderPath;
      const mediaUrl = renderPath ? `/api/capability-apps/${encodeURIComponent(app.app_id)}/projects/${encodeURIComponent(project.project_id)}/media/${renderPath.split("/").map(encodeURIComponent).join("/")}` : null;
      const stages = (project.stage_states ?? []).map((stage) => `<span class="status-label ${stage.status === "completed" ? "pass" : "local"}">${escapeHtml(stage.name)} · ${escapeHtml(stage.status)}</span>`).join(" ");
      const localNative = app.installation?.root?.startsWith("/Users/") ? `<a class="secondary" target="_blank" rel="noreferrer" href="${escapeHtml(app.native_ui.origin + app.native_ui.project_path_template.replace("{project_id}", encodeURIComponent(project.project_id)))}">打开原生工作台 ↗</a>` : `<span class="pill">原生工作台仅服务器回环访问</span>`;
      const renderJob = state?.render_job ?? null;
      return `<article class="panel" style="margin-top:12px"><div class="section-head"><div><span class="eyebrow">${escapeHtml(project.pipeline_type)} · ${escapeHtml(project.scene_count)} scenes</span><h3>${escapeHtml(project.title)}</h3><p class="help">${escapeHtml(project.project_id)} · ${escapeHtml(project.completed_count)} stages · ${escapeHtml(project.render_count)} render</p></div>${statusLabel(review?.status === "pass" ? "REVIEW PASS" : renderJob?.status ?? "PROJECT")}</div><div style="display:flex;gap:6px;flex-wrap:wrap;margin:12px 0">${stages}</div>${mediaUrl ? `<video controls preload="metadata" style="width:100%;max-height:520px;border-radius:12px;background:#000" src="${escapeHtml(mediaUrl)}"></video>` : `<div class="empty-state"><strong>等待独立应用产出主文件</strong></div>`}<div class="summary-grid" style="margin-top:12px"><div class="summary-card"><span>输出</span><strong>${escapeHtml(report?.resolution ?? "—")}</strong><small>${escapeHtml(report?.duration_seconds ?? "—")} 秒 · ${escapeHtml(report?.codec ?? "—")}</small></div><div class="summary-card"><span>质量复核</span><strong>${escapeHtml(review?.status?.toUpperCase() ?? "—")}</strong><small>${escapeHtml(review?.metadata?.audio_loudness?.integrated_lufs ?? "—")} LUFS</small></div><div class="summary-card"><span>远端渲染</span><strong>${escapeHtml(renderJob?.status ?? "未启动")}</strong><small>${escapeHtml(renderJob?.review_status ?? renderJob?.error ?? "—")}</small></div></div><div class="panel" style="margin-top:12px"><strong>远端回归素材</strong><p class="help">选择已标准命名的 8 个素材：origin.mp4、arrival.mp4、park-day.mp4、park-blue.mp4、park-night.mp4、park-render.jpg、jinhu-logo.jpg、music.mp3。浏览器按 512KB 分片上传并校验 SHA-256。</p><input type="file" multiple data-capability-files data-app-id="${escapeHtml(app.app_id)}" data-project-id="${escapeHtml(project.project_id)}" accept="video/*,audio/*,image/*"><div class="button-row" style="margin-top:10px"><button class="secondary" type="button" data-capability-upload>上传并校验素材</button><button class="primary-action" type="button" data-capability-render data-app-id="${escapeHtml(app.app_id)}" data-project-id="${escapeHtml(project.project_id)}">启动独立应用渲染</button></div></div><div class="button-row" style="margin-top:12px">${localNative}${receipt ? `<span class="status-label pass">OpenMontage 已接收</span>` : `<button class="secondary" type="button" data-capability-handoff data-app-id="${escapeHtml(app.app_id)}" data-project-id="${escapeHtml(project.project_id)}" data-title="${escapeHtml(project.title)}">登记任务交接</button>`}</div><p class="help" data-capability-message="${escapeHtml(project.project_id)}">Studio 不创建第二套任务、队列或运行时；创作执行与产物仍归 ${escapeHtml(app.name)} 所有。</p></article>`;
    }).join("");
    return `<section><div class="section-head"><div><span class="eyebrow">Independent Managed App · ${escapeHtml(app.license)}</span><h2>${escapeHtml(app.name)}</h2><p class="help">完整独立部署，Studio 只做 HANDOFF_ONLY 与 READ_ONLY_PROJECTION。</p></div>${statusLabel(app.status)}</div><div class="panel"><div class="simple-list"><div class="simple-row"><strong>安装</strong><span>${app.installation?.manifest_found ? escapeHtml(app.installation.root) : "未发现"}</span></div><div class="simple-row"><strong>固定版本</strong><span>${escapeHtml(app.deployment?.commit ?? "local development")}</span></div><div class="simple-row"><strong>运行边界</strong><span>${escapeHtml(app.boundary.studio_orchestration)} / ${escapeHtml(app.boundary.progress)}</span></div></div></div>${projects || `<div class="empty-state"><strong>应用已连接，尚无项目</strong></div>`}</section>`;
  }).join("");
  return `<section class="hero"><div><span class="eyebrow">Federated Creative Infrastructure</span><h1>能力应用中心</h1><p>专业系统独立运行，Studio 提供统一身份、任务交接、进度观察和成片回投。</p></div><span class="status-label pass">NO DUPLICATE RUNTIME</span></section><section><div class="summary-grid"><div class="summary-card"><span>独立应用</span><strong>${apps.length}</strong></div><div class="summary-card"><span>已就绪</span><strong>${ready}</strong></div><div class="summary-card"><span>外部项目</span><strong>${projectCount}</strong></div></div><div class="panel" style="margin-top:12px"><strong>当前金湖回归任务的素材事实</strong><p class="help">使用此前提供的园区视频、建筑效果图、Logo 和音乐进行重新剪辑与品牌合成；不是重新生成园区实拍视频，也不是未经处理的顺序拼接。</p></div></section>${panels}<script>(()=>{const json=async response=>{const body=await response.json();if(!response.ok)throw new Error(body.reason||body.status||'请求失败');return body;},hex=buffer=>[...new Uint8Array(buffer)].map(value=>value.toString(16).padStart(2,'0')).join('');document.querySelectorAll('[data-capability-handoff]').forEach(button=>button.onclick=async()=>{const message=document.querySelector('[data-capability-message="'+CSS.escape(button.dataset.projectId)+'"]');button.disabled=true;try{const body=await json(await fetch('/api/capability-apps/'+encodeURIComponent(button.dataset.appId)+'/handoffs',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({mode:'observe_existing',projectId:button.dataset.projectId,title:button.dataset.title,pipelineType:'hybrid',goal:'由 Studio 登记并观察独立 OpenMontage 项目'})}));message.textContent='交接已登记：'+body.handoff.handoff_id;location.reload();}catch(error){message.textContent=error.message;}finally{button.disabled=false;}});document.querySelectorAll('[data-capability-upload]').forEach(button=>button.onclick=async()=>{const input=button.closest('.panel').querySelector('[data-capability-files]'),message=document.querySelector('[data-capability-message="'+CSS.escape(input.dataset.projectId)+'"]'),files=[...input.files];if(!files.length){message.textContent='请先选择素材文件。';return;}button.disabled=true;try{for(let fileIndex=0;fileIndex<files.length;fileIndex++){const file=files[fileIndex],sha256=hex(await crypto.subtle.digest('SHA-256',await file.arrayBuffer()));message.textContent='上传 '+file.name+'（'+(fileIndex+1)+'/'+files.length+'）';const upload=await json(await fetch('/api/capability-apps/'+encodeURIComponent(input.dataset.appId)+'/uploads',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({projectId:input.dataset.projectId,name:file.name,size:file.size,sha256})}));for(let index=0;index<upload.chunk_count;index++){const chunk=file.slice(index*upload.chunk_size,Math.min(file.size,(index+1)*upload.chunk_size));await json(await fetch('/api/capability-apps/'+encodeURIComponent(input.dataset.appId)+'/uploads/'+encodeURIComponent(upload.upload_id)+'/chunks/'+index,{method:'PUT',headers:{'content-type':'application/octet-stream'},body:chunk}));}await json(await fetch('/api/capability-apps/'+encodeURIComponent(input.dataset.appId)+'/uploads/'+encodeURIComponent(upload.upload_id)+'/complete',{method:'POST'}));}message.textContent='全部素材已远端校验并交给 OpenMontage。';}catch(error){message.textContent=error.message;}finally{button.disabled=false;}});document.querySelectorAll('[data-capability-render]').forEach(button=>button.onclick=async()=>{const message=document.querySelector('[data-capability-message="'+CSS.escape(button.dataset.projectId)+'"]');button.disabled=true;try{await json(await fetch('/api/capability-apps/'+encodeURIComponent(button.dataset.appId)+'/projects/'+encodeURIComponent(button.dataset.projectId)+'/render',{method:'POST'}));message.textContent='远端渲染已启动，刷新页面查看状态。';setTimeout(()=>location.reload(),2000);}catch(error){message.textContent=error.message;button.disabled=false;}});})();</script>`;
}
...
function pageGraphicDesign(data) {
  ...
  <section class="grid"><div class="panel"><div class="section-head small"><h3>Photoshop UXP</h3><span class="status-label blocked">未激活</span></div><p>插件、任务 Schema、PSD/PNG/PDF 构建和产物校验已经具备；真实执行需要 Photoshop 运行、UXP 插件加载和交互会话确认。</p></div><div class="panel"><div class="section-head small"><h3>设计知识资源</h3><span class="status-label pass">多源已注册</span></div><p>设计系统、前端构图、UI/UX 检索和交互动效资源均按固定提交只读加载；不会覆盖项目设计系统或复制品牌身份。</p></div><div class="panel"><div class="section-head small"><h3>交付标准</h3><span class="status-label ready">可验证</span></div><p>网站与应用必须提交桌面/移动端视觉验收；平面设计默认要求可编辑 PSD、PNG 预览、PDF、设计说明和产物清单。</p></div></section>
...
export async function renderConsolePage(pathname = "/", auth = null, options = {}) {
  ...
  const contentById = {
    ...
    capabilityApps: () => pageCapabilityApps(data),
