...
function pageMyWork() {
  ...
  <section class="summary-grid" id="work-summary"></section><section><div class="section-head"><div><h2>工作队列</h2><p class="help">每项工作都可返回对应业务单据。</p></div></div><div id="work-list" class="panel"><div class="empty-state"><strong>正在读取工作</strong></div></div></section>
  <script>(()=>{const list=document.getElementById('work-list'),summary=document.getElementById('work-summary'),esc=v=>String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));async function load(){const response=await fetch('/api/work',{cache:'no-store'}),body=await response.json();if(!response.ok)throw new Error(body.reason||'工作队列不可用');const cards=[['全部工作',body.summary.total],['我的待办',body.summary.human],['Agent 工作',body.summary.agent],['阻塞/待审批',body.summary.blocked+body.summary.waitingApproval]];summary.innerHTML=cards.map(x=>'<div class="summary-card"><span>'+x[0]+'</span><strong>'+x[1]+'</strong></div>').join('');list.innerHTML=body.items.length?'<table><thead><tr><th>工作</th><th>业务单据</th><th>执行者</th><th>状态</th><th></th></tr></thead><tbody>'+body.items.map(item=>'<tr><td><strong>'+esc(item.title)+'</strong></td><td>'+esc(item.businessObject.displayKey)+'</td><td>'+(item.assignmentType==='AGENT'?'智能助手':'业务人员')+'</td><td>'+esc(item.status)+'</td><td><a class="quiet-link" href="'+esc(item.businessObject.href)+'">打开单据 →</a></td></tr>').join('')+'</tbody></table>':'<div class="empty-state"><strong>当前没有待办</strong>业务系统分配的人工和 Agent 工作会出现在这里。</div>';}document.getElementById('work-refresh').onclick=()=>load().catch(error=>list.textContent=error.message);load().catch(error=>list.textContent=error.message);})();</script>`;
}

function pageBusinessApplication(application, data) {
  const appJson=JSON.stringify(application).replaceAll("<","\\u003c"),canOperate=(data.renderAuth.capabilities??[]).some(value=>value==="*"||value==="business.operate");
  const typeOptions=application.objectTypes.map(type=>`<option value="${escapeHtml(type.id)}">${escapeHtml(type.name)}</option>`).join("");
  return `<section class="product-hero"><div><span class="eyebrow">${escapeHtml(application.nameEn)}</span><h2>${escapeHtml(application.name)}</h2><p>面向${escapeHtml(application.personas.join("、"))}的独立业务应用。业务记录是正式事实，智能助手只负责分析、分配和推动流程。</p></div><span class="status-label pass">● 业务应用</span></section>
  ...
  <script>(()=>{const app=${appJson},canOperate=${canOperate},records=document.getElementById('business-records'),message=document.getElementById('business-message'),esc=v=>String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));const call=async(url,options={})=>{const response=await fetch(url,{headers:{'content-type':'application/json'},...options}),body=await response.json();if(!response.ok)throw new Error(body.reason||body.status||'请求失败');return body;};async function load(){const body=await call('/api/business/applications/'+encodeURIComponent(app.id)+'/records');records.innerHTML=body.records.length?'<table><thead><tr><th>编号</th><th>业务事项</th><th>类型</th><th>负责人</th><th>状态</th><th>操作</th></tr></thead><tbody>'+body.records.map(row=>'<tr id="record-'+esc(row.id)+'"><td>'+esc(row.displayKey)+'</td><td><strong>'+esc(row.title)+'</strong></td><td>'+esc(app.objectTypes.find(x=>x.id===row.objectType)?.name||row.objectType)+'</td><td>'+esc(row.ownerId)+'</td><td>'+esc(row.status)+'</td><td>'+(canOperate&&row.status!=='COMPLETED'?'<div class="button-row"><button class="secondary" data-assign="HUMAN" data-id="'+esc(row.id)+'">分配给我</button><button class="secondary" data-assign="AGENT" data-id="'+esc(row.id)+'">委派智能助手</button><button class="primary" data-complete="'+esc(row.id)+'" data-version="'+row.version+'">完成</button></div>':'—')+'</td></tr>').join('')+'</tbody></table>':'<div class="empty-state"><strong>还没有业务记录</strong>从新建业务记录开始；即使智能 Runtime 关闭，业务台账仍然可用。</div>';}if(canOperate){document.getElementById('business-create').onclick=async()=>{try{await call('/api/business/applications/'+encodeURIComponent(app.id)+'/records',{method:'POST',body:JSON.stringify({objectType:document.getElementById('business-type').value,title:document.getElementById('business-title').value,displayKey:document.getElementById('business-ref').value})});message.textContent='业务记录已保存。';document.getElementById('business-title').value='';await load();}catch(error){message.textContent=error.message;}};records.onclick=async event=>{const assign=event.target.closest('[data-assign]'),complete=event.target.closest('[data-complete]');try{if(assign)await call('/api/business/applications/'+encodeURIComponent(app.id)+'/work',{method:'POST',body:JSON.stringify({businessObjectId:assign.dataset.id,assignmentType:assign.dataset.assign,assigneeId:assign.dataset.assign==='AGENT'?'agent-business-operator':'${escapeHtml(data.renderAuth.user?.user_id??"unknown")}'})});if(complete)await call('/api/business/applications/'+encodeURIComponent(app.id)+'/records/'+encodeURIComponent(complete.dataset.complete)+'/transition',{method:'POST',body:JSON.stringify({expectedVersion:Number(complete.dataset.version),status:'COMPLETED'})});await load();}catch(error){message.textContent=error.message;}};}document.getElementById('business-refresh').onclick=()=>load().catch(error=>records.textContent=error.message);load().catch(error=>records.textContent=error.message);})();</script>`;
}
