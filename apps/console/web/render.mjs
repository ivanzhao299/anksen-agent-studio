...
function pagePortfolio(data) {
  ...
  document.getElementById('portfolio-create').addEventListener('click',async()=>{const button=document.getElementById('portfolio-create'),selected=[...document.getElementById('portfolio-application').selectedOptions].map(option=>option.value),payload={workstreams:selected.map((applicationId,index)=>({applicationId,dependsOn:index?[selected[index-1]]:[]})),projectId:document.getElementById('portfolio-project').value,goal:document.getElementById('portfolio-goal').value.trim(),scheduleMode:document.getElementById('portfolio-mode').value,intervalMinutes:Number(document.getElementById('portfolio-interval').value),maxCycles:Number(document.getElementById('portfolio-cycles').value),maxTasks:Number(document.getElementById('portfolio-tasks').value),maxTokenEstimate:Number(document.getElementById('portfolio-tokens').value),maxRuntimeMinutes:Number(document.getElementById('portfolio-runtime').value)};if(!payload.goal){message.textContent='请先填写长期目标。';return;}if(!selected.length){message.textContent='请至少选择一个业务平台。';return;}button.disabled=true;try{await call('/api/portfolio/campaigns',{method:'POST',body:JSON.stringify(payload)});message.textContent='跨平台草稿已生成。请检查依赖、Skill、Agent 与预算后批准。';await refresh();}catch(error){message.textContent=error.message;}finally{button.disabled=false;}});
