...
function pageMyWork(data) {
  ...
  return `${canControl?"":"<style>#work-list [data-work-action]{display:none}</style>"}<section class="product-hero"><div><span class="eyebrow">My Work</span><h2>${canManage?"工作区工作":"我的工作"}</h2><p>${canManage?"集中处置当前工作区的业务待办、Agent 工作、阻塞与人工接管。":"集中处理分配给我的业务待办、我委派给 Agent 的工作、待审批事项、阻塞和分析结果。"}这里展示业务工作，不暴露内部租约、进程或 Fencing 信息。</p></div><button id="work-refresh" class="secondary" type="button">刷新</button></section>
