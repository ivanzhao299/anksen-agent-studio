...
function pageBusinessApplication(application, data) {
  ...
  async function openDelegation(id){const plan=await call('/api/business/applications/'+encodeURIComponent(app.id)+'/records/'+encodeURIComponent(id)+'/delegation-preview');delegationState.recordId=id;delegationState.version=plan.businessObject.version;document.getElementById('business-delegation-title').textContent=plan.businessObject.title;delegationContent.innerHTML='<section class="summary-grid"><div class="summary-card"><span>预检状态</span><strong>'+esc(plan.status)+'</strong><small>对象 v'+esc(plan.businessObject.version)+'</small></div><div class="summary-card"><span>智能流程</span><strong>'+esc(plan.workflow.domainName)+'</strong><small>'+esc(plan.workflow.definitionId)+'</small></div><div class="summary-card"><span>执行 Runtime</span><strong>'+esc(plan.policy.executionRuntime)+'</strong><small>真实 Runtime 保持关闭</small></div><div class="summary-card"><span>结果回写</span><strong>'+esc(plan.workflow.expectedWritebackStatus)+'</strong><small>版本校验后回写</small></div></section><section class="panel"><h3>执行目标</h3><p>'+esc(plan.goal)+'</p></section><section class="panel"><h3>Workflow · Skill · Agent · Runner</h3><div class="simple-list">'+plan.stages.map(stage=>'<div class="simple-row"><div><strong>'+esc(stage.stageId)+' · '+esc(stage.title)+'</strong><small>'+esc(stage.businessSkillId)+' / '+esc(stage.skillType)+'</small></div><div><span class="status-label '+(stage.status==='READY'?'pass':'blocked')+'">'+esc(stage.status)+'</span><small>Agent '+esc(stage.agentId||'—')+' · Runner '+esc(stage.workerKey||'—')+' ('+esc(stage.runnerMode)+')</small></div></div>').join('')+'</div></section>'+(plan.blockedReasons.length?'<section class="attention-card panel"><strong>当前不能委派</strong><p>'+esc(plan.blockedReasons.join(' · '))+'</p></section>':'');delegationConfirm.disabled=plan.status!=='READY';delegationDialog.showModal();}
