...
const dispatchPortfolioInitiative = async ({ campaign, initiative }) => {
  ...
  try {
    ...
    const service = new PersistentDomainWorkflowService(pool, { registry: portfolioRegistry }),scope={organizationId:campaign.organizationId??"studio-org",workspaceId:campaign.workspaceId??"studio-workspace",projectId:campaign.projectId,applicationId:initiative.applicationId,domainId:initiative.domainId,userId:campaign.approvedBy},app=getEnterpriseApplication(initiative.applicationId),catalogApplication=domainCenterSummary().applications.find(item=>item.id===initiative.applicationId),domain=catalogApplication?.domains.find(item=>item.id===initiative.domainId);let businessTaskBinding=null,capabilityProtocol=null,record=null;
    if(initiative.businessObject){record=await businessApplicationStore.getRecord(app.id,initiative.businessObject.id,scope);if(!record)throw Object.assign(new Error("PORTFOLIO_BUSINESS_OBJECT_NOT_FOUND"),{code:"PORTFOLIO_BUSINESS_OBJECT_NOT_FOUND"});if(record.schema.workflowDomainId!==initiative.domainId)throw Object.assign(new Error("PORTFOLIO_BUSINESS_OBJECT_DOMAIN_MISMATCH"),{code:"PORTFOLIO_BUSINESS_OBJECT_DOMAIN_MISMATCH"});const stage=domain.workflow[0];businessTaskBinding=createBusinessTaskBinding({scope,businessObject:{systemId:app.id,objectType:record.objectType,objectId:record.id,version:record.version,displayKey:record.displayKey,href:`${app.path}?record=${record.id}`},workflow:{definitionId:`${domain.id}-workflow`,definitionVersion:"1",instanceId:initiative.id,stageId:stage.key},skill:{businessSkillId:stage.businessSkillId,skillId:stage.skillType,skillType:stage.skillType,requiredCapabilities:[stage.skillType],riskLevel:domain.riskLevel??"LOW"},execution:{assignmentPolicy:"CAPABILITY",preferredRuntimeId:"controlled-stub",memoryScopeKey:`${app.id}:${record.objectType}:${record.id}`},writeback:{operation:"NONE",expectedObjectVersion:record.version,eventType:`portfolio.${initiative.domainId}.execution-completed`}});capabilityProtocol=buildBusinessCapabilityProtocol({application:app,domain,record,registry:portfolioRegistry});}
    const submitted = await service.submit({ sessionKey: initiative.sessionKey, goal: initiative.title, explicitDomainId: initiative.domainId,businessTaskBinding,capabilityProtocol,scope });
    ...
    return { status: report?.sessionStatus ?? "FAILED", report:{...report,businessObject:record?{applicationId:app.id,objectType:record.objectType,objectId:record.id,displayKey:record.displayKey,version:record.version,href:`${app.path}?record=${record.id}`}:null,capabilityContractId:capabilityProtocol?.contractId??null,capabilityContractHash:capabilityProtocol?.contractHash??null} };
