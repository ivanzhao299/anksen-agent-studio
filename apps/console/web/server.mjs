...
const runBusinessWork = async ({ app, record, item, actor }) => {
  const catalog=domainCenterSummary(),application=catalog.applications.find(value=>value.id===app.id),domain=application?.domains?.find(value=>value.id===record.schema.workflowDomainId);
  ...
  try{
    ...
    const service=new PersistentDomainWorkflowService(pool,{registry:portfolioRegistry}),submitted=await service.submit({sessionKey:`business-work:${item.id}:v${item.version}`,goal:item.title,explicitDomainId:domain.id,businessTaskBinding:binding,scope:binding.scope}),runningItem=await businessApplicationStore.attachWorkflow(item.id,{goalId:submitted.goal?.id??submitted.session.goal_id,sessionId:submitted.session.id,report:null,status:"RUNNING"});
    await service.runDaemon({pollMs:5,idleTimeoutMs:50,maxRuntimeMs:5000});
    const report=await service.night.loadReport(submitted.session.id),succeeded=report?.sessionStatus==="SUCCEEDED",reviewStatus=record.schema.agentReviewStatus,businessStatus=succeeded&&record.availableTransitions.includes(reviewStatus)?reviewStatus:null,workStatus=succeeded?(businessStatus??"COMPLETED"):"BLOCKED";
    const completed=await businessApplicationStore.completeWorkflow(item.id,{goalId:submitted.goal?.id??submitted.session.goal_id,sessionId:submitted.session.id,report,workStatus,businessStatus,expectedObjectVersion:record.version,expectedWorkVersion:runningItem.version,actorId:actor.userId});
    return{...completed,workflowStatus:workStatus,report};
...
const server = createServer(async (request, response) => {
  try {
    ...
    if (request.method === "GET" && pathname === "/api/work") {
      ...
      sendJson(response,200,{...work,approvals,canApprove,summary:{...work.summary,pendingApprovals:approvals.length},backend:businessRuntime.backend});
      return;
    }
    ...
    if(request.method==="POST"&&businessWorkControl){const access=await evaluateConsoleActionAccess(accessBundle,{action_id:"business-work-control",risk:"MEDIUM"},{user_context:accessContext});if(access.status!=="ALLOW"){sendJson(response,403,access);return;}try{const body=await readJsonBody(request),actor={organizationId:accessContext.organization_id??"studio-org",workspaceId:accessContext.workspace_id??"studio-workspace",userId:accessContext.user?.user_id},item=await businessApplicationStore.controlWorkItem(decodeURIComponent(businessWorkControl[1]),body,actor);let execution=null;if(item.assignmentType==="AGENT"&&["RESUME","RETRY","REASSIGN"].includes(String(body.action??"").toUpperCase())){const app=getEnterpriseApplication(item.applicationId),record=app?await businessApplicationStore.getRecord(app.id,item.businessObject.objectId,actor):null;if(!app||!record)throw Object.assign(new Error("BUSINESS_RECORD_NOT_FOUND"),{code:"BUSINESS_RECORD_NOT_FOUND"});execution=await runBusinessWork({app,record,item,actor});}sendJson(response,200,{item,execution});}catch(error){sendJson(response,409,{status:error.code??"BUSINESS_WORK_CONTROL_FAILED",reason:error.message});}return;}
