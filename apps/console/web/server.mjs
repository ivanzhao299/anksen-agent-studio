...
import { PostgresGrowthFeatureFlagStore } from "../../../packages/domain-center/lib/postgres-growth-feature-flag-store.mjs";
import { PostgresGrowthRuntimeGateEvidence } from "../../../packages/domain-center/lib/postgres-growth-runtime-gate-evidence.mjs";
...
const growthDeliveryStore = businessRuntime.pool ? new PostgresGrowthDeliveryStore({ pool: businessRuntime.pool, authorizeRetry: async input => input.accessDecision?.status === "ALLOW" && input.accessDecision?.required_capabilities?.includes("business.work.control"), authorizeReconciliation: async input => input.accessDecision?.status === "ALLOW" && input.accessDecision?.required_capabilities?.includes("proposal.approve") }) : null;
const growthIdentityReviewStore = businessRuntime.pool ? new PostgresGrowthIdentityReviewStore({ pool: businessRuntime.pool, authorizeDecision: async input => input.accessDecision?.status === "ALLOW" && input.accessDecision?.required_capabilities?.includes("proposal.approve") }) : null;
...
const growthFeatureFlagStore = businessRuntime.pool ? new PostgresGrowthFeatureFlagStore({ pool: businessRuntime.pool }) : null;
const growthRuntimeGateEvidence = businessRuntime.pool ? new PostgresGrowthRuntimeGateEvidence({ pool: businessRuntime.pool }) : null;
...
const loadGrowthSourceApproval=async scope=>{if(!businessSourceGovernance)return{status:"NOT_READY",source:"NOT_CONNECTED",summary:{total:0,ready:0,blocked:0}};try{return await businessSourceGovernance.tenantReadiness(scope,{applicationId:"ai-growth-sales-platform"});}catch(error){if(error?.code==="42P01"||error?.code==="42703")return{status:"NOT_READY",source:"MIGRATION_REQUIRED",summary:{total:0,ready:0,blocked:0}};throw error;}};
const loadGrowthProductionFeatureFlag=async scope=>{if(!growthFeatureFlagStore)return{status:"NOT_CONFIGURED",enabled:false,source:"NOT_CONNECTED"};try{return await growthFeatureFlagStore.readiness(scope);}catch(error){if(error?.code==="42P01")return{status:"NOT_CONFIGURED",enabled:false,source:"MIGRATION_REQUIRED"};throw error;}};
const loadGrowthRuntimeGate=async scope=>{if(!growthRuntimeGateEvidence)return{status:"NOT_BOUND",source:"NOT_CONNECTED"};try{return await growthRuntimeGateEvidence.readiness(scope,kingTurfTenantPack.metadata.runtimeActivationBinding);}catch(error){if(error?.code==="42P01")return{status:"NOT_READY",source:"MIGRATION_REQUIRED"};throw error;}};
const loadGrowthExplicitProductionAuthorization=async scope=>{if(!growthConnectorActivationGate)return{summary:{productionAuthorizationCovered:false},source:"NOT_CONNECTED"};try{return{...await growthConnectorActivationGate.listPreflights({scope}),source:"GOVERNED_CONNECTOR_ACTIVATION_PREFLIGHTS"};}catch(error){if(error?.code==="42P01"||error?.code==="42703")return{summary:{productionAuthorizationCovered:false},source:"MIGRATION_REQUIRED"};throw error;}};
...
const server = createServer(async (request, response) => {
  try {
    ...
    if(request.method==="GET"&&pathname==="/api/business/growth-sales/pilot-readiness"){
      const access=evaluateConsoleRouteAccess("growthSales",accessContext);if(!access.allowed){sendJson(response,403,access);return;}const scope={organizationId:kingTurfTenantPack.organizationId,workspaceId:kingTurfTenantPack.workspaceId,tenantId:kingTurfTenantPack.tenantId},scopeMismatch=(accessContext.organization_id&&accessContext.organization_id!==scope.organizationId)||(accessContext.workspace_id&&accessContext.workspace_id!==scope.workspaceId)||(accessContext.tenant_id&&accessContext.tenant_id!==scope.tenantId);if(scopeMismatch){sendJson(response,404,{status:"GROWTH_PILOT_TENANT_NOT_CONFIGURED"});return;}const [operations,connectors,productionOperations,sourceApproval,productionFeatureFlag,runtimeGate,activationAuthorization]=await Promise.all([loadGrowthPilotOperations(scope),loadGrowthPilotConnectors(scope),growthProductionOperationsPolicy.status(),loadGrowthSourceApproval(scope),loadGrowthProductionFeatureFlag(scope),loadGrowthRuntimeGate(scope),loadGrowthExplicitProductionAuthorization(scope)]),governance={...kingTurfPilotReadinessInput.governance,dataOwnerApproval:sourceApproval.status==='READY',productionFeatureFlag:productionFeatureFlag.enabled===true,productionOperationsPolicyAuthorized:productionOperations.status==='AUTHORIZED',productionOperationsPolicySource:productionOperations.gateId,runtimeActivationGatePassed:runtimeGate.status==='READY',explicitProductionAuthorization:activationAuthorization.summary.productionAuthorizationCovered===true},report=assessGrowthPilotReadiness({tenantPack:kingTurfTenantPack,...kingTurfPilotReadinessInput,governance,connectors,operations});sendJson(response,200,{organizationId:report.organizationId,workspaceId:report.workspaceId,tenantId:report.tenantId,status:report.status,implementationReady:report.implementationReady,activationReady:report.activationReady,implementation:report.implementation,activation:report.activation,blockers:report.blockers,safety:report.safety,generatedAt:report.generatedAt,source:"VERSIONED_PILOT_EVIDENCE_WITH_LIVE_OPERATIONS",operationsSource:{delivery:operations.source,identityReview:operations.identityReviewSource,publishingCredential:connectors.find(item=>item.kind==='PUBLISHING')?.evidenceSource??"NOT_CONNECTED",dataOwnerApproval:sourceApproval.source,productionFeatureFlag:productionFeatureFlag.source,productionOperations:productionOperations.gateId??"MISSING",runtimeActivationGate:runtimeGate.source,explicitProductionAuthorization:activationAuthorization.source}});return;
    }
