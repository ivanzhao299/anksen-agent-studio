...
const loadGrowthPilotConnectors=async scope=>{const connectors=kingTurfPilotReadinessInput.connectors.map(item=>({...item}));if(!growthConnectorEvidence)return connectors;try{const evidence=await growthConnectorEvidence.publishing(scope,{platforms:kingTurfTenantPack.metadata.publishingAccountPlatforms??[]}),index=connectors.findIndex(item=>item.kind==='PUBLISHING');if(index>=0)connectors[index]={...connectors[index],credentialReferenceConfigured:evidence.credentialReferenceConfigured,health:evidence.health,evidenceSource:evidence.source,credentialValuesRead:evidence.credentialValuesRead};return connectors;}catch(error){if(error?.code==="42P01")return connectors;throw error;}};
