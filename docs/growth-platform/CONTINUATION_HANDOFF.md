...
8. The Console now provides a read-only activation-preflight projection for production-operations handoff without exposing authorization or credential references. Keep execution unexposed until current-user authorization and an operational disable/rollback path are proven end to end.
