...
8. The Console provides a read-only activation-preflight projection for production-operations handoff without exposing authorization or credential references. The Gate now proves exact-version, independently authorized emergency disable with immutable incident evidence, but neither activation nor disable has a Console endpoint. Keep both unexposed until a named production-operator persona and end-to-end current-user authorization are proven.
