...
Latest local evidence (2026-08-27): the complete command exited zero; its Domain/reference phase passed 156/156 tests, the isolated PostgreSQL persistence smoke returned `SUCCEEDED`, and the Console projection phase passed 4/4 tests. This is local evidence for the unpushed branch head, not a claim about PR #35's older remote head or production readiness.
...
`deploy-business-data.sh` rejects a symlinked or non-deployment-user-owned data directory and symlink/non-regular `.env` or database-URL paths before credential generation/source/write, then preserves `0700/0600` modes; existing `.env` permissions are also reset to `0600` before source. The script is an explicit PR/push Growth CI path and its preflight is locked by the configuration suite; no deployment was run while adding this guard.
