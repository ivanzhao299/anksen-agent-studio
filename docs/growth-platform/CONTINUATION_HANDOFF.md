...
8. The Console provides a read-only activation-preflight projection for production-operations handoff without exposing authorization or credential references. The Gate proves Access Center authorization through the separate `growth_production_operator`, exact-version activation and emergency disable with immutable incident evidence, but neither operation has a Console endpoint. Keep both unexposed until authenticated endpoint CSRF/session controls and explicit production authorization are proven end to end.
