...
8. The Console provides a read-only activation-preflight projection for production-operations handoff without exposing authorization or credential references. The Gate proves Access Center authorization through the separate `growth_production_operator`, but a second Production Ops seam defaults to deny in accordance with the global disabled policy. Exact-version activation/disable are exercised only by an explicit local governance fixture; neither operation has a Console endpoint. Keep both unexposed until authoritative production authorization and authenticated endpoint controls are proven end to end.
