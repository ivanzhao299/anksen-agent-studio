...
- `PRODUCTION_FEATURE_FLAG` is backed by migration 019 and a tenant-scoped CAS store, not user/plan beta features. A missing or expired flag is disabled; enabling requires a bounded authorization reference and expiry, while disable remains available by exact version. Immutable events retain only a hash of the authorization reference. Readiness exposes no reference and the Console has no flag mutation endpoint.
