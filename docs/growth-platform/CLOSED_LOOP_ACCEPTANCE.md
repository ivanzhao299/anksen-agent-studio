...
- The named `growth_production_operator` role is the separated execution persona. It may pass Access Center checks for governed activation and CRITICAL emergency disable, but has no `business.operate`, `proposal.approve`, access-management or Runtime-execution capability; the Gate integration test uses that real policy evaluation rather than a hard-coded allow callback.
