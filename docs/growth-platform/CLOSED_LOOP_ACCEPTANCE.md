...
- Business Runtime and Growth initialization share `growth_schema_migration` under advisory lock `16012027`. Each 012–021 script runs once per database, preventing repeat trigger/ALTER DDL from deadlocking with business transactions in concurrent Console/test processes. Migration 021 also creates its trigger conditionally rather than dropping and recreating it.
