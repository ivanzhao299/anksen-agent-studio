...
- The health-probe seam accepts only adapters declaring `READ_ONLY_HEALTH_PROBE`, rejects disabled/stale-version bindings, passes a Credential Reference identifier rather than resolving a value, and returns explicit `externalWritesPerformed: false`. Raw probe evidence is hashed before persistence and omitted from audit/readiness projections.
