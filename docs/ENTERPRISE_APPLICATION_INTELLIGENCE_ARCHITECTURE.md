...
Business records are not isolated task cards. The transactional store persists typed, auditable relationships between records in the same organization, workspace, and application. The first supported chains are objective → KPI, recruitment → onboarding, budget → expense, lead → opportunity, BOM → work order, and enterprise → service order. Arbitrary or reversed combinations fail closed; repeated creation is idempotent; cross-tenant targets are invisible. Both ends of a relationship expose the other record and its current state in the conventional record detail surface.
