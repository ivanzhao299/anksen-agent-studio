...
This bootstrap establishes only:
...
It does not copy implementation from `jinhu-smart-park/ops/agent-orchestrator`.
...
1. Add project connector loading.
2. Port pure schemas from the current orchestrator after review.
3. Add CLI parity tests against a fixture project.
4. Add read-only Console shell.
5. Only then plan controlled migration of reusable logic.
