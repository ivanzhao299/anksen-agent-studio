...
export async function readBoundedJson(response,maxResponseBytes,errorPrefix){
  ...
  if(!/^application\/(?:[a-z0-9.+-]*\+)?json(?:\s*;|$)/.test(contentType))throw Object.assign(new Error(`${errorPrefix}_RESPONSE_CONTENT_TYPE_INVALID`),{code:`${errorPrefix}_RESPONSE_CONTENT_TYPE_INVALID`,retryable:false});
