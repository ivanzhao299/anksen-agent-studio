...
export async function resolveCredentialWithTimeout(resolver,input,timeoutMs,errorPrefix){const controller=new AbortController();let timer;try{return await Promise.race([Promise.resolve().then(()=>resolver({...input,signal:controller.signal})),new Promise((_,reject)=>{timer=setTimeout(()=>{controller.abort();reject(Object.assign(new Error(`${errorPrefix}_CREDENTIAL_TIMEOUT`),{code:`${errorPrefix}_CREDENTIAL_TIMEOUT`,retryable:true}));},timeoutMs);})]);}catch(error){if(error?.code===`${errorPrefix}_CREDENTIAL_TIMEOUT`)throw error;throw Object.assign(new Error(`${errorPrefix}_CREDENTIAL_RESOLUTION_FAILED`),{code:`${errorPrefix}_CREDENTIAL_RESOLUTION_FAILED`,retryable:true});}finally{clearTimeout(timer);}}
