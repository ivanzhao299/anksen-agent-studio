...
export async function readBoundedJson(response,maxResponseBytes,errorPrefix){
  ...
  if(Number.isFinite(declared)&&declared>maxResponseBytes)throw Object.assign(new Error(`${errorPrefix}_RESPONSE_TOO_LARGE`),{code:`${errorPrefix}_RESPONSE_TOO_LARGE`,retryable:false});
  if(!response.body?.getReader){const text=await response.text();if(Buffer.byteLength(text)>maxResponseBytes)throw Object.assign(new Error(`${errorPrefix}_RESPONSE_TOO_LARGE`),{code:`${errorPrefix}_RESPONSE_TOO_LARGE`,retryable:false});try{return JSON.parse(text);}catch{return{};}}
  ...
  const text=new TextDecoder().decode(Buffer.concat(chunks.map(value=>Buffer.from(value))));try{return JSON.parse(text);}catch{return{};}
}
