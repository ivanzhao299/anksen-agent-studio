...
test('bounded JSON reader rejects coerced response metadata',async()=>{for(const values of [[{toString:()=>"application/json"},null],['application/json',{toString:()=>"2"}]]){let cancellations=0;const response={headers:{get(name){return name==='content-type'?values[0]:values[1];}},body:{async cancel(){cancellations+=1;}}};await assert.rejects(()=>readBoundedJson(response,64,'OFFICIAL_API'),error=>['OFFICIAL_API_RESPONSE_CONTENT_TYPE_INVALID','OFFICIAL_API_RESPONSE_CONTENT_LENGTH_INVALID'].includes(error.code)&&error.retryable===false);assert.equal(cancellations,1);}});
