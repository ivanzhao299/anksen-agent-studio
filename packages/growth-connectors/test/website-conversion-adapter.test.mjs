...
test('website secret resolution is bounded before HMAC',async()=>{const body=JSON.stringify({eventType:'PAGE_VIEW',externalId:'secret-bound'}),headers={'x-growth-event-id':'secret-bound','x-growth-timestamp':timestamp,'x-growth-signature':sign(body,'secret-bound')},oversized=createWebsiteConversionAdapter({domain:'example.com',secretProvider:async()=>'x'.repeat(4097),clock:()=>new Date(Number(timestamp)*1000).toISOString()});await assert.rejects(()=>oversized.ingestWebhook({rawBody:body,headers}),/SECRET_UNAVAILABLE/);const hanging=createWebsiteConversionAdapter({domain:'example.com',secretProvider:async()=>new Promise(()=>{}),clock:()=>new Date(Number(timestamp)*1000).toISOString(),secretResolutionTimeoutMs:100});await assert.rejects(()=>hanging.ingestWebhook({rawBody:body,headers}),/SECRET_UNAVAILABLE/);assert.throws(()=>createWebsiteConversionAdapter({domain:'example.com',secretProvider:async()=>secret,secretResolutionTimeoutMs:99}),/secretResolutionTimeoutMs/);});
