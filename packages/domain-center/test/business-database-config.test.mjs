...
test("business database URL files are bounded absolute private real files",async()=>{const directory=await mkdtemp(join(tmpdir(),'anksen-db-url-')),path=join(directory,'database-url'),link=join(directory,'database-url-link'),url='postgresql://business:password@127.0.0.1/anksen_business';try{await writeFile(path,url,{mode:0o600});assert.equal(resolveBusinessDatabaseUrl({BUSINESS_DATABASE_URL_FILE:path}),url);await chmod(path,0o644);assert.throws(()=>resolveBusinessDatabaseUrl({BUSINESS_DATABASE_URL_FILE:path}),/URL_FILE_INVALID/);await chmod(path,0o600);await symlink(path,link);assert.throws(()=>resolveBusinessDatabaseUrl({BUSINESS_DATABASE_URL_FILE:link}),/URL_FILE_INVALID/);for(const value of ['   ','x'.repeat(4097)]){await writeFile(path,value);assert.throws(()=>resolveBusinessDatabaseUrl({BUSINESS_DATABASE_URL_FILE:path}),/URL_FILE_INVALID/);}assert.throws(()=>resolveBusinessDatabaseUrl({BUSINESS_DATABASE_URL_FILE:'relative/database-url'}),/URL_FILE_INVALID/);assert.throws(()=>resolveBusinessDatabaseUrl({BUSINESS_DATABASE_URL_FILE:directory}),/URL_FILE_INVALID/);}finally{await rm(directory,{recursive:true,force:true});}});
