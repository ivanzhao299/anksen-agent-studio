...
import { assertBusinessDatabaseUrl,createBusinessApplicationRuntime, resolveBusinessDatabasePoolMax,resolveBusinessDatabaseRemoteAccess,resolveBusinessDatabaseTimeoutMs,resolveBusinessDatabaseUrl } from "../lib/business-database.mjs";
...
test("business database configuration is local, explicit and credential-backed", () => {
  ...
  assert.throws(()=>assertBusinessDatabaseUrl(`postgresql://business:${encodeURIComponent('😀'.repeat(300))}@127.0.0.1/anksen_business`),/CREDENTIAL_INVALID/);
...
test("business database remote access is an exact environment boolean",()=>{assert.equal(resolveBusinessDatabaseRemoteAccess({}),false);assert.equal(resolveBusinessDatabaseRemoteAccess({BUSINESS_DATABASE_ALLOW_REMOTE:'true'}),true);for(const value of ['TRUE','1','yes'])assert.throws(()=>resolveBusinessDatabaseRemoteAccess({BUSINESS_DATABASE_ALLOW_REMOTE:value}),/ENV_INVALID/);let calls=0;const env=Object.defineProperty({},'BUSINESS_DATABASE_ALLOW_REMOTE',{enumerable:true,get(){calls+=1;return'true';}});assert.throws(()=>resolveBusinessDatabaseRemoteAccess(env),/ENV_INVALID/);assert.equal(calls,0);});
...
test("Office deployment provisions and verifies isolated business PostgreSQL before restart", async () => {
  ...
  assert.match(deployData,/\[\[ ! -O "\$data_dir" \]\]/);
  ...
  assert.ok(deployData.indexOf('[[ ! -O "$data_dir" ]]')<deployData.indexOf('source "$data_env"'));
