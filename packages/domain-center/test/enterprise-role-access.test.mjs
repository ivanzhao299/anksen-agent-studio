...
test("sales reviewer can approve content and publish plans without publication or Runtime authority",async()=>{
  ...
  assert.equal((await evaluateConsoleActionAccess({policy},{action_id:"business-approval-decision",risk:"MEDIUM"},{user_context:context})).status,"ALLOW");
  assert.equal((await evaluateConsoleActionAccess({policy},{action_id:"growth-identity-review-decision",risk:"MEDIUM"},{user_context:context})).status,"ALLOW");
  for(const actionId of ["growth-delivery-retry","growth-delivery-reconcile","growth-connector-activate","growth-connector-disable"]){assert.equal((await evaluateConsoleActionAccess({policy},{action_id:actionId,risk:actionId.startsWith("growth-connector-")?"CRITICAL":"MEDIUM"},{user_context:context})).status,"DENY");}
...
test("sales operator can request an eligible retry but cannot assert reconciliation, identity resolution or connector state",async()=>{const role=policy.roles.find(item=>item.role_id==="sales_operator"),context=contextFor(role);assert.equal((await evaluateConsoleActionAccess({policy},{action_id:"growth-delivery-retry",risk:"MEDIUM"},{user_context:context})).status,"ALLOW");for(const actionId of ["growth-delivery-reconcile","growth-identity-review-decision","growth-connector-activate","growth-connector-disable"])assert.equal((await evaluateConsoleActionAccess({policy},{action_id:actionId,risk:actionId.startsWith("growth-connector-")?"CRITICAL":"MEDIUM"},{user_context:context})).status,"DENY");});

test("growth production operator can execute governed connector state changes without self-approval or business editing",async()=>{const role=policy.roles.find(item=>item.role_id==="growth_production_operator"),context=contextFor(role);assert.equal(evaluateConsoleRouteAccess("growthSales",context).allowed,true);for(const actionId of ["growth-connector-activate","growth-connector-disable"])assert.equal((await evaluateConsoleActionAccess({policy},{action_id:actionId,risk:"CRITICAL"},{user_context:context})).status,"ALLOW");for(const actionId of ["business-record-create","business-record-update","business-record-transition","business-approval-request","business-approval-decision","growth-identity-review-decision","access-user-create","agent-real-execute"])assert.equal((await evaluateConsoleActionAccess({policy},{action_id:actionId,risk:"MEDIUM"},{user_context:context})).status,"DENY",actionId);assert.equal(role.capabilities.includes("proposal.approve"),false);assert.equal(role.capabilities.includes("business.operate"),false);assert.equal(role.capabilities.includes("access.manage"),false);assert.equal(role.capabilities.includes("autopilot.execute.local"),false);});

test("strategy reviewer can approve governed reviews without cross-application or Runtime authority",async()=>{
  const role=policy.roles.find(item=>item.role_id==="strategy_reviewer"),context=contextFor(role);
  assert.equal(evaluateConsoleRouteAccess("strategy",context).allowed,true);assert.equal(evaluateConsoleRouteAccess("finance",context).allowed,false);
  assert.equal((await evaluateConsoleActionAccess({policy},{action_id:"business-approval-decision",risk:"MEDIUM"},{user_context:context})).status,"ALLOW");
  for(const actionId of ["business-work-control","development-job-approve","production-operation-request","identity-owner-bootstrap"]){assert.equal((await evaluateConsoleActionAccess({policy},{action_id:actionId,risk:"MEDIUM"},{user_context:context})).status,"DENY");}
  assert.equal(role.capabilities.includes("autopilot.plan"),false);assert.equal(role.capabilities.includes("autopilot.execute.local"),false);assert.equal(role.capabilities.includes("access.manage"),false);
});

test("Smart Park reviewer can accept service evidence without Agent or cross-application authority",async()=>{
  const role=policy.roles.find(item=>item.role_id==="smart_park_reviewer"),context=contextFor(role);
  assert.equal(evaluateConsoleRouteAccess("smartPark",context).allowed,true);assert.equal(evaluateConsoleRouteAccess("finance",context).allowed,false);
  assert.equal((await evaluateConsoleActionAccess({policy},{action_id:"business-approval-decision",risk:"MEDIUM"},{user_context:context})).status,"ALLOW");
  for(const actionId of ["business-work-control","development-job-approve","production-operation-request","identity-owner-bootstrap"]){assert.equal((await evaluateConsoleActionAccess({policy},{action_id:actionId,risk:"MEDIUM"},{user_context:context})).status,"DENY");}
  assert.equal(role.capabilities.includes("autopilot.plan"),false);assert.equal(role.capabilities.includes("autopilot.execute.local"),false);assert.equal(role.capabilities.includes("access.manage"),false);
});
