...
test("sales reviewer can approve content and publish plans without publication or Runtime authority",async()=>{
  ...
  for(const actionId of ["growth-delivery-retry","growth-delivery-reconcile"]){assert.equal((await evaluateConsoleActionAccess({policy},{action_id:actionId,risk:"MEDIUM"},{user_context:context})).status,"DENY");}
...
test("sales operator can request an eligible retry but cannot assert reconciliation or identity resolution",async()=>{const role=policy.roles.find(item=>item.role_id==="sales_operator"),context=contextFor(role);assert.equal((await evaluateConsoleActionAccess({policy},{action_id:"growth-delivery-retry",risk:"MEDIUM"},{user_context:context})).status,"ALLOW");for(const actionId of ["growth-delivery-reconcile","growth-identity-review-decision"])assert.equal((await evaluateConsoleActionAccess({policy},{action_id:actionId,risk:"MEDIUM"},{user_context:context})).status,"DENY");});
