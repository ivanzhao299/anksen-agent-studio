...
test("named business operators can use their own conventional lifecycle but not other applications",async()=>{
  for(const [roleId,route] of Object.entries(routes)){
    ...
    for(const actionId of ["business-record-create","business-record-update","business-record-transition","business-record-relate","business-related-record-create","business-approval-request","business-work-assign","business-work-control"]){assert.equal((await evaluateConsoleActionAccess({policy},{action_id:actionId,risk:actionId==="business-record-create"||actionId==="business-work-assign"?"LOW":"MEDIUM"},{user_context:context})).status,"ALLOW",`${roleId}:${actionId}`);}
