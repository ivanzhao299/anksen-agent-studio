...
test("sales reviewer can approve content and publish plans without publication or Runtime authority",async()=>{
  ...
  for(const actionId of ["growth-delivery-retry","growth-delivery-reconcile","growth-connector-activate"]){assert.equal((await evaluateConsoleActionAccess({policy},{action_id:actionId,risk:actionId==="growth-connector-activate"?"CRITICAL":"MEDIUM"},{user_context:context})).status,"DENY");}
...
test("sales operator can request an eligible retry but cannot assert reconciliation, identity resolution or activation",async()=>{const role=policy.roles.find(item=>item.role_id==="sales_operator"),context=contextFor(role);assert.equal((await evaluateConsoleActionAccess({policy},{action_id:"growth-delivery-retry",risk:"MEDIUM"},{user_context:context})).status,"ALLOW");for(const actionId of ["growth-delivery-reconcile","growth-identity-review-decision","growth-connector-activate"])assert.equal((await evaluateConsoleActionAccess({policy},{action_id:actionId,risk:actionId==="growth-connector-activate"?"CRITICAL":"MEDIUM"},{user_context:context})).status,"DENY");});
