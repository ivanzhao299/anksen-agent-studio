...
test("sales operator can request an eligible retry but cannot assert reconciliation",async()=>{const role=policy.roles.find(item=>item.role_id==="sales_operator"),context=contextFor(role);assert.equal((await evaluateConsoleActionAccess({policy},{action_id:"growth-delivery-retry",risk:"MEDIUM"},{user_context:context})).status,"ALLOW");assert.equal((await evaluateConsoleActionAccess({policy},{action_id:"growth-delivery-reconcile",risk:"MEDIUM"},{user_context:context})).status,"DENY");});
