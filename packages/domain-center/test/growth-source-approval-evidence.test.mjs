...
test("growth source ingestion validates its envelope before SQL",async()=>{let queries=0;const store=new PostgresBusinessDataConnectorStore({pool:{async query(){queries+=1;throw new Error('must not query');}}}),scope={organizationId:'org',workspaceId:'growth'},record={sourceRecordKey:'lead-1',objectType:'lead',displayKey:'LEAD-1',title:'Lead 1',ownerId:'sales',fields:{}},base={idempotencyKey:'batch-1',evidenceRef:'source://growth/batch-1',observedAt:'2026-08-26T00:00:00Z',records:[record]};await assert.rejects(()=>store.ingest(123,base,scope),error=>error.code==='BUSINESS_SYNC_CONNECTOR_INVALID');await assert.rejects(()=>store.ingest('connector-1',{...base,idempotencyKey:{toString:()=>"batch-1"}},scope),error=>error.code==='BUSINESS_SYNC_IDEMPOTENCY_INVALID');await assert.rejects(()=>store.ingest('connector-1',{...base,records:[{...record,title:{toString:()=>"Lead 1"}}]},scope),error=>error.code==='BUSINESS_SYNC_TITLE_INVALID');await assert.rejects(()=>store.ingest('connector-1',{...base,evidenceRef:'token=raw-secret'},scope),error=>error.code==='BUSINESS_SYNC_EVIDENCE_INVALID');assert.equal(queries,0);});
