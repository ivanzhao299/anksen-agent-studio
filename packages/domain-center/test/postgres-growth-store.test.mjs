...
test('customer 360 never sums revenue across currencies',async()=>{const pool={async query(sql){if(sql.startsWith('SELECT * FROM growth_lead'))return{rowCount:1,rows:[{id:'lead-1',score:null}]};if(sql.includes('FROM growth_revenue_attribution'))return{rowCount:3,rows:[{currency:'USD',amount:'100'},{currency:'EUR',amount:'50'},{currency:'USD',amount:'25'}]};return{rowCount:0,rows:[]};}},store=new PostgresGrowthStore({pool}),view=await store.customer360({scope,leadId:'lead-1'});assert.deepEqual(view.revenueByCurrency,{EUR:50,USD:125});assert.equal(view.totalRevenue,null);});
