...
test("growth migration inspection is read-only and fail closed on drift",async()=>{const fixturePath=fileURLToPath(new URL("./fixtures/999_growth_checksum_fixture.up.sql",import.meta.url)),calls=[],client={async query(sql){calls.push(sql);return{rows:[{name:"999_growth_checksum_fixture.up.sql",checksum:"0".repeat(64)}]};}},drift=await inspectGrowthMigrations(client,[fixturePath]);assert.equal(drift.status,"BLOCKED");assert.deepEqual(drift.summary,{total:1,applied:0,pending:0,drift:1,legacyChecksumMissing:0,unexpectedApplied:0});assert.equal(calls.length,1);const pending=await inspectGrowthMigrations({async query(){return{rows:[]};}},[fixturePath]);assert.equal(pending.status,"PENDING");assert.equal(pending.items[0].actualChecksum,null);});
...
test("growth migration status command stays read-only with an injected pool",async()=>{let calls=0;const report=await growthMigrationStatus({pool:{async query(sql){calls+=1;assert.match(sql,/^SELECT name,checksum/);return{rows:[]};}}});assert.equal(report.status,"PENDING");assert.equal(report.summary.total,growthMigrationPaths.length);assert.equal(calls,1);});
