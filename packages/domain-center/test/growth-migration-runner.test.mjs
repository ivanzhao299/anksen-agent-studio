...
import {growthMigrationPaths,growthPrerequisiteMigrationPaths} from "../lib/growth-database.mjs";
...
test("strict growth migration apply blocks unexpected ledger entries",async()=>{await ensurePostgresFixture();const pool=createTestPool(),client=await pool.connect(),name='997_removed_growth_migration.up.sql';try{await withGrowthMigrationLock(client,async()=>{await applyGrowthMigrations(client,growthMigrationPaths,{strictManifest:true});await client.query('INSERT INTO growth_schema_migration(name,checksum) VALUES($1,$2) ON CONFLICT(name) DO UPDATE SET checksum=EXCLUDED.checksum',[name,'a'.repeat(64)]);try{await assert.rejects(()=>applyGrowthMigrations(client,growthMigrationPaths,{strictManifest:true}),error=>error.code==='GROWTH_SCHEMA_MIGRATION_MANIFEST_BLOCKED'&&error.summary.unexpectedApplied===1);}finally{await client.query('DELETE FROM growth_schema_migration WHERE name=$1',[name]);}});}finally{await client.query('DELETE FROM growth_schema_migration WHERE name=$1',[name]).catch(()=>{});client.release();await pool.end();}});
