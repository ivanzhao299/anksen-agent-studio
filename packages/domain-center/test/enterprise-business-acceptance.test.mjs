...
test("enterprise acceptance evaluator refuses incomplete runtime evidence",()=>{
  const scenarios=enterpriseAcceptanceScenarios.map(item=>({role:item.role,record:{persistedAfterRestart:true},tenantIsolation:true,humanWork:{count:1},agentWork:{status:"COMPLETED"},kernel:{sessionStatus:"SUCCEEDED",goalStatus:"SUCCEEDED",taskCount:4,attemptCount:4,runtimeExecutionCount:4}}));
