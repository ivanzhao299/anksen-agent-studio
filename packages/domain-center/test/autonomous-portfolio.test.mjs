...
test("Manufacturing ERP broad program proposes a formal record for every domain",async()=>{const{service}=await setup(),campaign=await service.create({applicationId:"intelligent-manufacturing-erp",goal:"完善制造 ERP 全流程"},{userId:"manufacturing-owner"});assert.equal(campaign.initiatives.length,14);assert.equal(campaign.businessObjectProposals.length,14);assert.equal(campaign.businessObjectProposals.every(item=>item.status==="NEEDS_INPUT"&&item.objectType),true);assert.equal(new Set(campaign.businessObjectProposals.map(item=>item.initiativeDomainId)).size,14);});

test("Growth and Sales broad program carries a formal record through all ten domains",async()=>{const{service}=await setup(),campaign=await service.create({applicationId:"ai-growth-sales-platform",goal:"完善增长销售全流程"},{userId:"sales-owner"});assert.equal(campaign.initiatives.length,10);assert.equal(campaign.businessObjectProposals.length,10);assert.equal(campaign.businessObjectProposals.every(item=>item.status==="NEEDS_INPUT"&&item.objectType),true);assert.deepEqual(campaign.businessObjectProposals.slice(-3).map(item=>item.objectType),["opportunity","transaction_handoff","customer_success_case"]);});
