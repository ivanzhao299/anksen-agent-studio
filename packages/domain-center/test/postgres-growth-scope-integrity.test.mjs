...
test('canonical growth relations and event idempotency fail closed across tenants',async()=>{
  ...
  await assert.rejects(()=>store.recordRevenue({scope:other,revenue:{id:`revenue-${suffix}`,opportunityId:opportunity.id,leadId,amount:100,currency:'USD',attributedAt:'2026-08-27T00:00:00Z'}}),error=>error.code==='GROWTH_REVENUE_TENANT_RELATION_REQUIRED_OR_CONFLICT');
  ...
  await assert.rejects(()=>pool.query(`INSERT INTO growth_lead(id,organization_id,workspace_id,tenant_id,source,status,person,company,external_refs) VALUES($1,$2,$3,$4,'DIRECT','not-valid','{}','{}','[]')`,[`invalid-lead-${suffix}`,scope.organizationId,scope.workspaceId,scope.tenantId]),error=>error.code==='23514');
  await assert.rejects(()=>pool.query(`INSERT INTO growth_engagement(id,organization_id,workspace_id,tenant_id,lead_id,kind,channel,payload,occurred_at) VALUES($1,$2,$3,$4,$5,'page-view','WEBSITE','[]',now())`,[`invalid-engagement-${suffix}`,scope.organizationId,scope.workspaceId,scope.tenantId,leadId]),error=>error.code==='23514');
  await assert.rejects(()=>pool.query(`INSERT INTO growth_score_snapshot(id,organization_id,workspace_id,tenant_id,lead_id,score_type,value,confidence,factors,dimensions,model_version,policy_version,calculated_at) VALUES($1,$2,$3,$4,$5,'lead-quality',80,1,'{}','[]','bad model','v1',now())`,[`invalid-score-${suffix}`,scope.organizationId,scope.workspaceId,scope.tenantId,leadId]),error=>error.code==='23514');
  await assert.rejects(()=>pool.query(`INSERT INTO growth_opportunity(id,organization_id,workspace_id,tenant_id,lead_id,stage,score,downstream_ref) VALUES($1,$2,$3,$4,$5,'qualified',101,'[]')`,[`invalid-opportunity-${suffix}`,scope.organizationId,scope.workspaceId,scope.tenantId,leadId]),error=>error.code==='23514');
  await assert.rejects(()=>pool.query(`INSERT INTO growth_revenue_attribution(id,organization_id,workspace_id,tenant_id,opportunity_id,lead_id,amount,currency,attributed_at,metadata) VALUES($1,$2,$3,$4,$5,$6,-1,'usd',now(),'[]')`,[`invalid-revenue-${suffix}`,scope.organizationId,scope.workspaceId,scope.tenantId,opportunity.id,leadId]),error=>error.code==='23514');
  await assert.rejects(()=>pool.query(`INSERT INTO growth_connector_binding(id,organization_id,workspace_id,tenant_id,kind,adapter_id,transport,credential_reference_id,endpoint_host,last_actor_id) VALUES($1,$2,$3,$4,'WEBSITE_INBOUND','bad adapter','SIGNED_WEBHOOK','plain-ref','localhost','bad actor')`,[`invalid-binding-${suffix}`,scope.organizationId,scope.workspaceId,scope.tenantId]),error=>error.code==='23514');
