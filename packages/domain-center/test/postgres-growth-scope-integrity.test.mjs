...
test('canonical growth relations and event idempotency fail closed across tenants',async()=>{
  ...
  await assert.rejects(()=>pool.query(`INSERT INTO growth_identity(id,organization_id,workspace_id,tenant_id,lead_id,identity_type,normalized_value,source) VALUES($1,$2,$3,$4,$5,'SOCIAL','unbounded identity','DIRECT')`,[randomUUID(),scope.organizationId,scope.workspaceId,scope.tenantId,leadId]),error=>error.code==='23514');
