...
test('delivery controls validate before authorization or SQL',async()=>{let authorizationCalls=0,queryCalls=0;const scope={organizationId:'org',workspaceId:'growth',tenantId:'tenant'},store=new PostgresGrowthDeliveryStore({pool:{async query(){queryCalls+=1;throw new Error('query must not run');}},clock:()=>new Date(Number.NaN),authorizeRetry:async()=>{authorizationCalls+=1;return true;},authorizeReconciliation:async()=>{authorizationCalls+=1;return true;}});await assert.rejects(()=>store.register({scope,idempotencyKey:'publish-1',adapterId:'official-v1',assetRef:'asset-1'}),error=>error.code==='DELIVERY_CLOCK_INVALID');await assert.rejects(()=>store.requestRetry({scope,id:'bad operation',expectedVersion:1,actorId:'operator'}),error=>error.code==='DELIVERY_OPERATION_REFERENCE_INVALID');await assert.rejects(()=>store.requestRetry({scope,id:'delivery-1',expectedVersion:0,actorId:'operator'}),error=>error.code==='DELIVERY_VERSION_INVALID');await assert.rejects(()=>store.reconcile({scope,id:'delivery-1',expectedVersion:1,observedExternalId:'token=secret',actorId:'reviewer'}),error=>error.code==='DELIVERY_OBSERVED_EXTERNAL_ID_REFERENCE_INVALID');assert.equal(authorizationCalls,0);assert.equal(queryCalls,0);});
