...
test("AI Growth and Sales has conventional records across content, acquisition, transaction and service",()=>{const application=enterpriseApplications.find(item=>item.id==="ai-growth-sales-platform"),covered=new Set(application.objectTypes.map(item=>getBusinessObjectDefinition(application.id,item.id).workflowDomainId));assert.deepEqual([...getStudioApplication(application.id).domainIds].filter(domainId=>!covered.has(domainId)),[]);assert.equal(application.objectTypes.length,11);assert.match(businessWorkflowGoal(application.id,{objectType:"video_matrix",title:"产品视频矩阵"}),/不自动发布/);assert.match(businessWorkflowGoal(application.id,{objectType:"transaction_handoff",title:"交易交接"}),/不代替交易系统下单/);});
