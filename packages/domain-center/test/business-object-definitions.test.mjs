...
import { businessRelationContracts } from "../lib/business-relation-definitions.mjs";
...
test("Manufacturing ERP has one conventional operational object for all fourteen workflow domains",()=>{const application=enterpriseApplications.find(item=>item.id==="intelligent-manufacturing-erp"),covered=new Set(application.objectTypes.map(item=>getBusinessObjectDefinition(application.id,item.id).workflowDomainId));assert.deepEqual([...getStudioApplication(application.id).domainIds].filter(domainId=>!covered.has(domainId)),[]);assert.equal(application.objectTypes.length,14);assert.match(businessWorkflowGoal(application.id,{objectType:"manufacturing_cost",title:"2026-07 成本"}),/财务凭证/);assert.match(businessWorkflowGoal(application.id,{objectType:"maintenance_order",title:"设备维修"}),/不远程控制设备/);});
test("Manufacturing ERP relations cover order planning execution quality cost and traceability",()=>{const types=new Set(businessRelationContracts.filter(item=>item.applicationId==="intelligent-manufacturing-erp").map(item=>item.relationType));for(const type of ["DRIVES_PLAN","SCHEDULES","USED_BY","GOVERNS","ALLOCATED_TO","EXECUTED_AS","GENERATES","COSTED_BY","TRACED_BY","EVIDENCES","SOURCES","REPLENISHES"])assert.ok(types.has(type));});

test("AI Growth and Sales has conventional records across content, acquisition, transaction and service",()=>{const application=enterpriseApplications.find(item=>item.id==="ai-growth-sales-platform"),covered=new Set(application.objectTypes.map(item=>getBusinessObjectDefinition(application.id,item.id).workflowDomainId));assert.deepEqual([...getStudioApplication(application.id).domainIds].filter(domainId=>!covered.has(domainId)),[]);assert.equal(application.objectTypes.length,12);assert.match(businessWorkflowGoal(application.id,{objectType:"video_matrix",title:"产品视频矩阵"}),/不自动发布/);assert.match(businessWorkflowGoal(application.id,{objectType:"transaction_handoff",title:"交易交接"}),/不代替交易系统下单/);assert.match(businessWorkflowGoal(application.id,{objectType:"connector_activation",title:"生产连接器"}),/审批本身不执行激活/);});
