...
export async function runEnterpriseBusinessAcceptance({pool,runId=randomUUID()}={}) {
  ...
  for(const scenario of enterpriseAcceptanceScenarios){
    ...
    scenarios.push({role:scenario.role,capability:app.capabilities[0],applicationId:scenario.applicationId,record:{id:record.id,initialStatus,status:persisted.status,version:persisted.version,persistedAfterRestart:Boolean(persisted)},tenantIsolation:foreign===null,businessChain:{relationType:scenario.related.relationType,direction:scenario.related.direction,relatedRecordId:related.id,relationCount:detail.relations.length,persistedAfterRestart:detail.relations.some(item=>item.record.id===related.id)},humanWork:{id:human.id,count:myWork.items.filter(item=>item.assignmentType==="HUMAN").length},agentWork:{id:agent.work.id,status:agent.work.status,assignmentType:agent.work.assignmentType,version:agent.work.version},kernel:{sessionKey:agent.sessionKey,domainId:agent.domainId,sessionStatus:agent.report.sessionStatus,goalStatus:agent.report.goalStatus,taskCount:agent.report.totalTasks,attemptCount:agent.report.attemptCount,runtimeExecutionCount:agent.report.runtimeExecutionCount},applicationReport});
  }
