...
export class PersistentGrowthIngestionService {
  ...
  async ingestWebsiteEvent({ scope: rawScope, event }) {
    ...
    try {
      ...
      const store = new PostgresGrowthStore({ pool: client,clock:()=>now });
      ...
      const scoring = createScoringEngine({ scope, policy: this.scoringPolicy, clock:()=>nowIso });
      ...
      ], occurredAt: event.provenance?.receivedAt ?? nowIso });
      const lead = createLead({ ...scope, leadId, source: existing?.source ?? event.source, status: existing?.status ?? 'NEW', person: { ...(existing?.person ?? {}), ...(event.person ?? {}) }, company: { ...(existing?.company ?? {}), ...(event.company ?? {}) }, externalRefs, score, createdAt: existing?.created_at ?? nowIso });
      ...
      await store.recordEngagement({ scope, engagement: { id: scopedEventId('eng', scope, event.eventId), leadId, kind: event.kind, channel: event.source, payload: { message: event.message, productRefs: event.productRefs, consent: event.consent }, occurredAt: event.provenance?.receivedAt ?? nowIso } });
      await store.recordScore({ scope, leadId, score: { ...score, scoreId: scopedEventId('score', scope, event.eventId) } });
      const auditEvent = createGrowthEvent({ ...scope, eventId: `evt_${randomUUID()}`, eventType: 'growth.engagement.received', subjectType: 'lead', subjectId: leadId, source: event.source, idempotencyKey, payload: { externalId: event.externalId, kind: event.kind, sourceDomain: event.sourceDomain }, occurredAt: event.provenance?.receivedAt ?? nowIso });
