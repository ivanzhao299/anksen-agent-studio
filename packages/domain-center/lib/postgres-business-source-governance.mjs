...
const governedConnector=(row,id,scope)=>{const invalid=()=>{throw fail("BUSINESS_SOURCE_CONNECTOR_EVIDENCE_INVALID");},value=governanceEnvelope(row,"BUSINESS_SOURCE_CONNECTOR_EVIDENCE_INVALID",new Set(["id","organization_id","workspace_id","application_id","source_system","connector_type","credential_reference_id","allowed_object_types","freshness_seconds","status","version","last_success_at","last_error_summary","created_by","created_at","updated_at"])),date=(input,{optional=false}={})=>{if(optional&&input==null)return null;if(typeof input!=="string"&&!(input instanceof Date))return invalid();const parsed=new Date(input);if(!Number.isFinite(parsed.getTime()))return invalid();return input;};if(value.id!==id||value.organization_id!==scope.organizationId||value.workspace_id!==scope.workspaceId||!["FIXTURE","API","SQL_READ_MODEL","WEBHOOK","FILE_IMPORT"].includes(value.connector_type)||!["ACTIVE","PAUSED","ERROR","REVOKED"].includes(value.status)||!Number.isInteger(value.version)||value.version<1||!Number.isInteger(value.freshness_seconds)||value.freshness_seconds<60||value.freshness_seconds>31536000||!Array.isArray(value.allowed_object_types)||value.allowed_object_types.length>100||value.allowed_object_types.some(item=>typeof item!=="string"))invalid();safeGovernanceRef(value.application_id,"CONNECTOR_APPLICATION",128);safeGovernanceRef(value.source_system,"CONNECTOR_SOURCE_SYSTEM",160);safeGovernanceRef(value.created_by,"CONNECTOR_CREATED_BY",128);if(value.credential_reference_id!=null)safeGovernanceRef(value.credential_reference_id,"CREDENTIAL_REFERENCE",240);date(value.last_success_at,{optional:true});date(value.created_at);date(value.updated_at);if(value.last_error_summary!=null&&(typeof value.last_error_summary!=="string"||value.last_error_summary.length>240||secretLike(value.last_error_summary)))invalid();return value;};
...
export class PostgresBusinessSourceGovernance {
  ...
  async connector(id, scope) {
    ...
    return governedConnector(row,safeId,s);
  }
  ...
  present(row) {
    row=governanceEnvelope(row,"BUSINESS_SOURCE_APPROVAL_EVIDENCE_INVALID",new Set(["id","connector_id","organization_id","workspace_id","tenant_id","data_owner_id","mapping_version","expires_at","status","requested_by","requested_at","decided_by","decided_at","decision_reason","version","sequence_id"]));const evidenceFail=()=>{throw fail("BUSINESS_SOURCE_APPROVAL_EVIDENCE_INVALID");},ref=(value,max=160,{optional=false}={})=>{if(optional&&value==null)return null;try{return safeGovernanceRef(value,"APPROVAL_EVIDENCE",max);}catch{return evidenceFail();}},date=(value,{optional=false}={})=>{if(optional&&value==null)return null;if(typeof value!=="string"&&!(value instanceof Date))return evidenceFail();const parsed=new Date(value);if(!Number.isFinite(parsed.getTime()))return evidenceFail();return parsed.toISOString();},reason=row.decision_reason;
