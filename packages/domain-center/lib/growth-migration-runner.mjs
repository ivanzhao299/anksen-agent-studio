...
export async function applyGrowthMigrations(client, migrationPaths,{strictManifest=false}={}) {
  await client.query(
  ...
  if(strictManifest){const inspection=await inspectGrowthMigrations(client,migrationPaths);if(inspection.status==="BLOCKED")throw Object.assign(new Error("GROWTH_SCHEMA_MIGRATION_MANIFEST_BLOCKED"),{code:"GROWTH_SCHEMA_MIGRATION_MANIFEST_BLOCKED",summary:inspection.summary});}
