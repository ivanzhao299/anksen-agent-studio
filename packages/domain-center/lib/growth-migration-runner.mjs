...
export async function inspectGrowthMigrations(client,migrationPaths){if(!client?.query)throw new TypeError("client is required");let rows;try{rows=(await client.query("SELECT name,checksum FROM growth_schema_migration ORDER BY name")).rows;}catch(error){if(error?.code!=="42P01")throw error;rows=[];}const byName=new Map(rows.map(row=>[row.name,row.checksum])),manifestNames=new Set(),items=[];for(const migrationPath of migrationPaths){const name=basename(migrationPath),sql=await readFile(migrationPath,"utf8"),expectedChecksum=checksum(sql),actualChecksum=byName.get(name),status=!byName.has(name)?"PENDING":!actualChecksum?"LEGACY_CHECKSUM_MISSING":actualChecksum===expectedChecksum?"APPLIED":"DRIFT";manifestNames.add(name);items.push({name,status,expectedChecksum,actualChecksum:actualChecksum??null});}for(const row of rows){if(!manifestNames.has(row.name))items.push({name:row.name,status:"UNEXPECTED_APPLIED",expectedChecksum:null,actualChecksum:row.checksum??null});}const summary={total:manifestNames.size,applied:items.filter(item=>item.status==="APPLIED").length,pending:items.filter(item=>item.status==="PENDING").length,drift:items.filter(item=>item.status==="DRIFT").length,legacyChecksumMissing:items.filter(item=>item.status==="LEGACY_CHECKSUM_MISSING").length,unexpectedApplied:items.filter(item=>item.status==="UNEXPECTED_APPLIED").length};return{status:summary.drift||summary.legacyChecksumMissing||summary.unexpectedApplied?"BLOCKED":summary.pending?"PENDING":"READY",summary,items};}
