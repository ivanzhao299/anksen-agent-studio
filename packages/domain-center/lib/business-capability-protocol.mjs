...
export function buildBusinessCapabilityProtocol({application,domain,record=null,registry}){
  const resolved=resolveDomainCapability(domain,registry),skills=new Map((registry.skillRegistry.skills??[]).map(item=>[item.skill_type,item])),agents=new Map((registry.agentRegistry.agents??[]).map(item=>[item.agent_id,item])),workers=new Map((registry.workerRegistry.workers??[]).map(item=>[item.worker_id,item])),professionalProfiles=new Map((registry.professionalCapabilities?.profiles??[]).map(item=>[item.profile_id,item])),errors=[],warnings=[];
  ...
  const stages=domain.workflow.map(stage=>{
    ...
    const professionalProfile=worker?.professional_profile_id?professionalProfiles.get(worker.professional_profile_id):null,capabilityEvidence=professionalProfile?{profileId:professionalProfile.profile_id,capabilityVersion:professionalProfile.capability_version??null,adapterId:professionalProfile.adapter_id??null,installationReadiness:professionalProfile.installation_readiness,executionReadiness:professionalProfile.execution_readiness,activated:professionalProfile.activated===true,evidenceHash:professionalProfile.evidence_hash,skillPackages:professionalProfile.skills.map(item=>({skill:item.skill,status:item.status})),tools:professionalProfile.tools.map(item=>({command:item.command,status:item.status,version:item.version})),credentialReferences:professionalProfile.credentials.map(item=>({referenceId:item.referenceId,status:item.status})),networkPolicy:professionalProfile.network_policy,sideEffectPolicy:professionalProfile.side_effect_policy,allowedOutputRoots:professionalProfile.allowed_output_roots,blockedReasons:professionalProfile.blocked_reasons,executionBlockedReasons:professionalProfile.execution_blocked_reasons}:null;
    return{stageId:stage.key,title:stage.title,dependsOn:stage.dependsOn,businessSkillId:stage.businessSkillId,implementationSkill:{skillId:skill?.skill_id??null,skillType:stage.skillType,riskLevel:skill?.risk_level??null,requiredInputs:skill?.required_inputs??[],expectedOutputTypes:skill?.expected_output_types??[]},agent:{agentId:preferred?.agent_id??null,displayName:preferred?.display_name??null,role:preferred?.role??null,eligibleAgentIds:eligibleAgents.map(item=>item.agent_id)},runner:{workerKey:worker?.worker_id??null,runtimeId:worker?.runtime_id??null,adapterId:worker?.adapter_id??null,professionalProfileId:worker?.professional_profile_id??null,capabilityEvidence,capabilityEvidenceHash:capabilityEvidence?hash(capabilityEvidence):null,mode:worker?.process_probe?.on_demand_ok?"ON_DEMAND":worker?"REGISTERED":"MISSING",requiredCapabilities:[stage.skillType]},status:stageErrors.length?"BLOCKED":"READY",blockedReasons:stageErrors};
  });
