...
export class PersistentBusinessWorkExecutor {
  ...
  async execute(item){let pool=null,ownsPool=false;try{
    ...
    const service=new PersistentDomainWorkflowService(pool,{registry:this.registry});let status=session.status,coordinator=null;
    try{coordinator=await pool.connect();await coordinator.query("SELECT pg_advisory_lock(hashtextextended('resident-business-kernel-scheduler',0))");for(let count=0;count<this.maxCycles&&status==="RUNNING";count+=1){try{const result=await service.runOnce(session.session_key);status=result.status??result.sessionStatus??result.report?.sessionStatus??"RUNNING";}catch(error){if(error?.code!=="SESSION_INACTIVE"&&error?.message!=="SESSION_INACTIVE")throw error;const current=(await pool.query("SELECT status FROM ad_night_shift_session WHERE id=$1",[item.sessionId])).rows[0];status=current?.status??"RUNNING";}}}finally{if(coordinator){await coordinator.query("SELECT pg_advisory_unlock(hashtextextended('resident-business-kernel-scheduler',0))").catch(()=>{});coordinator.release();}}
