...
export class PostgresGrowthStore {
  ...
  async upsertLead({ scope, lead }) {
    ...
    const result = await this.pool.query(
      `INSERT INTO growth_lead(id,organization_id,workspace_id,tenant_id,source,status,person,company,market_id,icp_id,external_refs,score,created_at,updated_at)
       ...
       RETURNING *`,
      [lead.leadId, s.organizationId, s.workspaceId, s.tenantId, lead.source, lead.status ?? 'NEW', lead.person ?? {}, lead.company ?? {}, lead.marketId ?? null, lead.icpId ?? null, lead.externalRefs ?? [], lead.score ?? null, lead.createdAt ?? new Date(), new Date()]
    );
  ...
  async appendEvent({ scope, event }) {
    ...
    await this.pool.query(
      ...
      [event.eventId, s.organizationId, s.workspaceId, s.tenantId, event.eventType, event.subjectType, event.subjectId, event.source, event.idempotencyKey, event.payload ?? {}, event.schemaVersion ?? 1, event.occurredAt]
    );
  }
  ...
  async recordEngagement({ scope, engagement }) {
    const s = assertTenantScope(scope);
    await this.pool.query(
      `INSERT INTO growth_engagement(id,organization_id,workspace_id,tenant_id,lead_id,kind,channel,payload,occurred_at) VALUES($1,$2,$3,$4,$5,$6,$7,$8,$9)`,
      [engagement.id, s.organizationId, s.workspaceId, s.tenantId, engagement.leadId, engagement.kind, engagement.channel ?? null, engagement.payload ?? {}, engagement.occurredAt]
    );
  }
  ...
  async upsertOpportunity({ scope, opportunity, downstreamRef = null }) {
    ...
    await this.pool.query(
      ...
      [opportunity.id, s.organizationId, s.workspaceId, s.tenantId, opportunity.leadId, opportunity.stage, opportunity.score ?? null, downstreamRef]
    );
  }
  ...
  async recordRevenue({ scope, revenue }) {
    ...
    await this.pool.query(
      ...
      [revenue.id, s.organizationId, s.workspaceId, s.tenantId, revenue.opportunityId, revenue.leadId, revenue.amount, revenue.currency, revenue.attributedAt, revenue.metadata ?? {}]
    );
  }
