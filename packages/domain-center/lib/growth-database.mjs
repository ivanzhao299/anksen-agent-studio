...
import { assertBusinessDatabaseUrl, resolveBusinessDatabasePoolMax,resolveBusinessDatabaseTimeoutMs,resolveBusinessDatabaseUrl } from './business-database.mjs';
...
export const growthMigrationPaths = Object.freeze([
  ...
  resolve(fileURLToPath(new URL('../../orchestrator-core/migrations/032_growth_lead_constraints.up.sql', import.meta.url))),
  resolve(fileURLToPath(new URL('../../orchestrator-core/migrations/033_growth_engagement_constraints.up.sql', import.meta.url))),
  resolve(fileURLToPath(new URL('../../orchestrator-core/migrations/034_growth_score_constraints.up.sql', import.meta.url))),
  resolve(fileURLToPath(new URL('../../orchestrator-core/migrations/035_growth_opportunity_revenue_constraints.up.sql', import.meta.url))),
  resolve(fileURLToPath(new URL('../../orchestrator-core/migrations/036_growth_connector_binding_constraints.up.sql', import.meta.url))),
  resolve(fileURLToPath(new URL('../../orchestrator-core/migrations/037_growth_identity_root_constraints.up.sql', import.meta.url))),
  resolve(fileURLToPath(new URL('../../orchestrator-core/migrations/038_growth_delivery_control_constraints.up.sql', import.meta.url))),
...
export async function createGrowthDatabaseRuntime({ env = process.env, pool = null } = {}) {
  ...
  const timeoutMs=pool?null:resolveBusinessDatabaseTimeoutMs(env);
