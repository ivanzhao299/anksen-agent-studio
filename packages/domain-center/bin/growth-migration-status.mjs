...
export async function growthMigrationStatus({env=process.env,pool=null}={}){
  ...
  try{const report=await inspectGrowthMigrations(database,growthMigrationPaths);return{schemaVersion:1,...report,safety:{readOnly:true,ddlExecuted:false,migrationsApplied:false,credentialsReturned:false}};}finally{if(!pool)await database.end();}
}
