...
import {assertBusinessDatabaseUrl,resolveBusinessDatabaseRemoteAccess,resolveBusinessDatabaseTimeoutMs,resolveBusinessDatabaseUrl} from '../lib/business-database.mjs';
...
export async function growthMigrationStatus({env=process.env,pool=null}={}){
  const timeoutMs=pool?null:resolveBusinessDatabaseTimeoutMs(env),database=pool??new Pool({connectionString:assertBusinessDatabaseUrl(resolveBusinessDatabaseUrl(env),{allowRemote:resolveBusinessDatabaseRemoteAccess(env)}),max:1,connectionTimeoutMillis:timeoutMs,query_timeout:timeoutMs,statement_timeout:timeoutMs,application_name:'anksen-growth-migration-status'});
