...
try{
  ...
  const submitted=await service.submit({sessionKey,goal:"优化仪表盘页面并完成测试",explicitDomainId:"experience-access",scope:{organizationId:"domain-smoke-org",workspaceId:"domain-smoke-workspace",projectId:"domain-smoke-project"}});
  ...
  const result={status:report.sessionStatus,goalStatus:report.goalStatus,domainId:submitted.workflow.domain.id,tasks:facts.length,attempts:report.attemptCount,claims:report.workerClaimCount,runtimeExecutions:report.runtimeExecutionCount,runner,facts};
  if(result.status!=="SUCCEEDED"||result.goalStatus!=="SUCCEEDED"||facts.length!==4||facts.some(fact=>fact.attempt_number!==1||fact.lease_status!=="RELEASED"||!fact.skill_type||!fact.agent_id||fact.planned_worker!=="local-codex-1"||fact.actual_worker!=="local-codex-1"))throw new Error("DOMAIN_RUNTIME_SMOKE_FAILED");
