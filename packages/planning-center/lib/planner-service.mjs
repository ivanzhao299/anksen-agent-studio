...
const templates = {
  ...
  VIDEO_FACTORY: [
    { suffix:"CREATIVE",title:"Define governed video strategy",capability:"planning",riskLevel:"LOW" },
    { suffix:"PROGRAMMATIC",title:"Render programmatic composition",capability:"programmatic_video",riskLevel:"MEDIUM",after:"CREATIVE" },
    { suffix:"TECHNICAL",title:"Render technical animation",capability:"technical_animation",riskLevel:"MEDIUM",after:"PROGRAMMATIC" },
    { suffix:"EDIT",title:"Assemble approved footage and animation",capability:"video_editing",riskLevel:"MEDIUM",after:"TECHNICAL" },
    { suffix:"QA",title:"Validate final media artifact",capability:"media_validation",riskLevel:"LOW",after:"EDIT" }
  ],
  GENERIC: [
...
export class RulePlannerEngine {
  selectTemplate(goal){const text=`${goal.title} ${goal.description??""}`.toLowerCase();if(/视频工厂|视频制作|视频矩阵|programmatic video|video factory|remotion|manim|video-use/.test(text))return"VIDEO_FACTORY";if(/代码|实现|开发|修复|重构|api|service|parser|schema|worker|preview|test|build|feature|bug/.test(text))return"SOFTWARE_DELIVERY";if(/文档|说明|手册|document|readme|guide/.test(text))return"DOCUMENTATION";return"GENERIC";}
