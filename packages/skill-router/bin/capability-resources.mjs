...
if(command==="inventory"){
  ...
  if(!target.startsWith(resolve(repoRoot,"runtime/capability-resources")+"/"))throw new Error("CAPABILITY_RESOURCE_TARGET_INVALID");
  ...
  if(result.status!==0)throw new Error(result.stderr||"CAPABILITY_RESOURCE_SYNC_FAILED");const inventory=await registry.inventory();console.log(JSON.stringify(inventory,null,2));const state=inventory.resources.find(item=>item.resource_id===definition.resource_id);if(state.integrity_status!=="PASS")throw new Error("CAPABILITY_RESOURCE_INTEGRITY_BLOCKED");
