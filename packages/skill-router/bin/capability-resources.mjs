...
if(command==="inventory"){
  ...
  if(result.status!==0)throw new Error(result.stderr||"CAPABILITY_RESOURCE_SYNC_FAILED");if(!resolve(target).startsWith(resolve(repoRoot,"runtime/capability-resources")))throw new Error("CAPABILITY_RESOURCE_TARGET_INVALID");const inventory=await registry.inventory();console.log(JSON.stringify(inventory,null,2));if(inventory.summary.ready!==inventory.summary.total)throw new Error("CAPABILITY_RESOURCE_INTEGRITY_BLOCKED");
