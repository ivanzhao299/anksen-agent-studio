...
const crmAdapter=defineBusinessIntegrationAdapter({id:'crm-mock',system:'DOWNSTREAM_CRM',capabilities:['RFQ'],requiresApproval:true,async execute(){return{externalId:'RFQ-AUTH-001',status:'OPEN'}}});const downstream=await handoffCommercialObject({scope,adapter:crmAdapter,objectType:'RFQ',lead,payload:{quantity:1000},operationId:'handoff-rfq-1',approvalRef:'approval:acceptance-handoff'});assert.equal(downstream.authoritative,true);
