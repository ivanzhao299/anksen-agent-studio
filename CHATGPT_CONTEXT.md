...
The capability center also includes the native `frontend-product-design` skill. It synthesizes commit-pinned, read-only references from OpenAI frontend composition (`30444aed500c00c85294d12074f6e3ee794f808a`), Emil Kowalski design engineering (`de33dbed000212b54400a33767d1e4d03654db2a`), and UI UX Pro Max (`abb7f2fd5a083fa1ff55c326a963ff0d95c33f99`). Frontend design work must establish an experience/visual thesis, prefer task-driven cardless composition, implement complete interaction states, and provide real desktop/mobile browser evidence. Database recommendations are candidates only; Studio warns on card/Bento, inaccessible, neon, glass, or other product-mismatched suggestions instead of applying them mechanically.

The governed development path supports opt-in Git worktree isolation: source changes are classified, a fetched remote baseline is pinned, execution uses a `codex/<task>` branch in an independent worktree, Agent ownership is recorded, and source or remote drift fails closed. The Avernet Provider compatibility gateway is implemented but disabled without a production credential reference and project allowlist; Avernet never becomes execution or release authority. MemPalace `3.7.0` is registered as a commit-pinned MIT resource and a disabled-by-default, non-authoritative derived-memory adapter with scoped paths, redaction, and separately gated writes.
